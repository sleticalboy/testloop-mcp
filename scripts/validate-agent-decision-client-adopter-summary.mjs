...
function usage() {
  console.error('Usage: node scripts/validate-agent-decision-client-adopter-summary.mjs [--json] [summary-json]');
  console.error('');
  console.error('Defaults:');
  console.error('  summary-json: docs/fixtures/agent-decision-client-adopter-summary/passed.json');
}
...
const summaryPath = path.resolve(
  positionalArgs[0] || 'docs/fixtures/agent-decision-client-adopter-summary/passed.json',
);
...
function emitFailures() {
  ...
  console.error('Agent decision client adopter summary validation failed:');
...
if (jsonMode) {
  ...
  console.log(`agent_decision_client_adopter_summary_status=passed fixture_count=${summary.fixture_count}`);
  console.log(`agent_decision_client_adopter_summary_agent_next_step=${summary.agent_next_step}`);
  console.log(`agent_decision_client_adopter_summary_should_accept=${summary.should_accept}`);
}
