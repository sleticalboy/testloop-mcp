...
function usage() {
  console.error('Usage: node scripts/validate-agent-decision-client-ci-install-summary.mjs [--json] [summary-json]');
  console.error('');
  console.error('Defaults:');
  console.error('  summary-json: docs/fixtures/agent-decision-client-ci-template-install-summary/passed.json');
}
...
const summaryPath = path.resolve(
  positionalArgs[0] || 'docs/fixtures/agent-decision-client-ci-template-install-summary/passed.json',
);
...
function emitFailures() {
  ...
  console.error('agent decision client CI install summary validation failed:');
...
if (typeof summary.installer_url !== 'string' || !summary.installer_url.includes('/scripts/install-agent-decision-client-ci-template.sh')) {
  failures.push(`${summaryPath}: installer_url must point to install-agent-decision-client-ci-template.sh`);
}
...
if (summary.contract_exit_code !== 0) {
  failures.push(`${summaryPath}: contract_exit_code must be 0`);
}
...
if (jsonMode) {
  ...
  console.log(`agent_decision_client_ci_install_summary_status=passed fixture_count=${summary.fixture_count}`);
  console.log(`agent_decision_client_ci_install_summary_decisions=${summary.decisions.join(',')}`);
}
