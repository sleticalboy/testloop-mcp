...
Environment:
  ...
  TESTLOOP_MCP_VERIFY_EXPECT_VERSION    Optional expected version, for example 0.5.5.
...
Examples:
  ...
  TESTLOOP_MCP_VERIFY_EXPECT_VERSION=0.5.5 scripts/showcase-agent-onboarding-report.sh
USAGE
}
