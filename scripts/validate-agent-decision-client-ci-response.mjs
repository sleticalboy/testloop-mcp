...
function usage() {
  console.error('Usage: node scripts/validate-agent-decision-client-ci-response.mjs [--json] [response-json]');
  console.error('');
  console.error('Defaults:');
  console.error('  response-json: docs/fixtures/agent-decision-client-ci-response/passed.json');
}
...
const responsePath = path.resolve(
  positionalArgs[0] || 'docs/fixtures/agent-decision-client-ci-response/passed.json',
);
...
function emitFailures() {
  ...
  console.error('agent decision client CI response validation failed:');
...
if (evidence.validator_exit_code !== 0) {
  failures.push(`${responsePath}: evidence.validator_exit_code must be 0`);
}
...
if (jsonMode) {
  ...
  console.log(`agent_decision_client_ci_response_status=passed agent_next_step=${response.agent_next_step}`);
  console.log(`agent_decision_client_ci_response_fixture_count=${evidence.fixture_count}`);
  console.log(`agent_decision_client_ci_response_decisions=${evidence.decisions.join(',')}`);
}
