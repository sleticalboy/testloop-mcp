...
function usage() {
  console.error('Usage: node scripts/validate-agent-decision-client-consumer-smoke-summary.mjs [--json] [summary-json]');
  console.error('');
  console.error('Defaults:');
  console.error('  summary-json: docs/fixtures/agent-decision-client-consumer-smoke-summary/passed.json');
}
...
function emitFailures() {
  ...
  console.error('agent decision client consumer smoke summary validation failed:');
...
if (summary.helper_ref !== 'v0.5.21') {
  failures.push(`${summaryPath}: helper_ref must be v0.5.21`);
}
...
for (const field of [
  ...
  if (summary[field] !== 0) {
    failures.push(`${summaryPath}: ${field} must be 0`);
  }
}
...
if (jsonMode) {
  ...
  console.log(`agent_decision_client_consumer_smoke_summary_status=passed fixture_count=${summary.fixture_count}`);
  console.log(`agent_decision_client_consumer_smoke_summary_decisions=${summary.decisions.join(',')}`);
}
