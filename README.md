...
`coverage=true` 时，`validate_coverage_task` 会尽量校验 `coverage_task.line_range` 是否真正被生成测试命中。当前支持 Jest/Vitest/Mocha 的 Istanbul `coverage/coverage-final.json`、Node.js 内置 `node-test` 的 TAP coverage raw output、pytest 项目根目录的 `coverage.json`、Rust/Cargo 的 LCOV，以及 Java/JUnit 的 JaCoCo XML。测试命令通过但目标行仍未覆盖时会返回 `failed/needs_better_input`，并在 `metadata` 中提供 `coverage_target_hit`、`coverage_hit_lines`、`coverage_missed_lines` 和 `coverage_miss_reason`。
