...
完整 action 执行建议见 [Agent Action 决策表](./docs/agent-action-guide.md)，结构化返回样例见 [validate_coverage_task 结构化返回样例](./docs/validate-coverage-task-samples.md)，真实 handler fixture 见 [真实结构化 fixture](./docs/fixtures.md)。可运行 `go run ./examples/agent-decision-demo` 查看最小客户端决策映射。
