...
它会临时创建外部 client，生成 workflow，运行 helper dry-run，并校验安装 summary、基础客户端 CI summary、导出的 fixture manifest 和 `agent-decision-fixtures-result.json`，同时生成可直接交给 Agent 的 `agent_response_json`。
