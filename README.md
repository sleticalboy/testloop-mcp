...
当前 `v0.5.21` Release 已提供 Linux amd64、Linux arm64、macOS arm64、Windows amd64 和 Windows arm64 二进制。手动下载示例：

```bash
curl -LO https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.21/testloop-mcp_v0.5.21_linux_amd64.tar.gz
curl -LO https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.21/testloop-mcp_v0.5.21_linux_amd64.tar.gz.sha256
sha256sum -c testloop-mcp_v0.5.21_linux_amd64.tar.gz.sha256
tar -xzf testloop-mcp_v0.5.21_linux_amd64.tar.gz
./testloop-mcp --help
```
...
```powershell
$arch = "amd64"
curl.exe -LO "https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.21/testloop-mcp_v0.5.21_windows_$arch.zip"
curl.exe -LO "https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.21/testloop-mcp_v0.5.21_windows_$arch.zip.sha256"
$expected = (Get-Content ".\testloop-mcp_v0.5.21_windows_$arch.zip.sha256").Split()[0]
$actual = (Get-FileHash ".\testloop-mcp_v0.5.21_windows_$arch.zip" -Algorithm SHA256).Hash.ToLower()
if ($actual -ne $expected) { throw "checksum mismatch" }
Expand-Archive ".\testloop-mcp_v0.5.21_windows_$arch.zip"
& ".\testloop-mcp_v0.5.21_windows_$arch\testloop-mcp.exe" --help
& ".\testloop-mcp_v0.5.21_windows_$arch\testloop-testgen.exe" --help
```
...
```bash
TESTLOOP_MCP_VERIFY_EXPECT_VERSION=0.5.21 scripts/verify-client-setup.sh /absolute/path/to/testloop-mcp
```
...
```bash
TESTLOOP_REPORT_EXPECT_VERSION=0.5.21 \
...
`coverage=true` 时，`validate_coverage_task` 会尽量校验 `coverage_task.line_range` 是否真正被生成测试命中。当前支持 Go coverprofile、Jest/Vitest/Mocha 的 Istanbul `coverage/coverage-final.json`、Node.js 内置 `node-test` 的 TAP coverage raw output、pytest 项目根目录的 `coverage.json`、Rust/Cargo 的 LCOV，以及 Java/JUnit 的 JaCoCo XML。Go 默认读取项目根目录的 `testloop-cover.out`，可通过 `TESTLOOP_GO_COVERPROFILE` 改为相对项目根或绝对路径。`metadata.coverage_target_hit_supported` 会明确当前框架是否支持目标行命中校验，`metadata.coverage_target_hit_supported_frameworks` 会返回当前支持矩阵。测试命令通过但目标行仍未覆盖时会返回 `failed/needs_better_input`，并在 `metadata` 中提供 `next_action_kind=coverage_target_miss`、`next_action_reason`、`coverage_target_hit`、`coverage_hit_lines` 和 `coverage_missed_lines`；旧字段 `coverage_miss_reason` 继续保留用于兼容。
...
`coverage=true` 时，Go 会追加 `-coverprofile <module>/testloop-cover.out` 并继续从测试输出回填 `coverage_percent`，也可通过 `TESTLOOP_GO_COVERPROFILE` 指向自定义 coverprofile 路径；Node.js 内置 `node-test` 会追加 `--experimental-test-coverage` 并从 TAP coverage report 的 `all files` 行回填 `coverage_percent`；Rust 会额外调用 `cargo tarpaulin --out Lcov --output-dir target/tarpaulin` 并回填 `coverage_percent`；Java Maven/Gradle 项目会执行 JaCoCo report 任务并从 XML 报告回填 `coverage_percent`。也可以通过 `parse_coverage` 直接解析已有 coverprofile、LCOV 或 JaCoCo XML 文件。
...
导出目录还会携带基础客户端 CI summary/response 的 schema、通过态/失败态 fixture、response renderer 和无依赖 validator；接入方不 checkout 整个仓库，也能离线固定 `agent_next_step` 的响应形状。
导出目录还包含无依赖 `package.json`，接入方可以直接运行 `npm test --silent`；需要固定基础 response 形状时，可运行 `npm run validate:client-summary --silent`、`npm run render:client-response --silent` 和 `npm run validate:client-response --silent`。
外部仓库可复制的基础消费样板见 [Agent decision client 接入方样板](./examples/agent-decision-client-adopter/README.md)，维护者可运行 `scripts/showcase-agent-decision-client-adopter.sh --json` 验证导出包、response renderer、validator 和接入方 helper 的完整基础闭环；summary 可用 `node scripts/validate-agent-decision-client-adopter-summary.mjs /path/to/agent-decision-client-adopter-summary.json` 校验，结构契约见 [agent-decision-client-adopter-summary.schema.json](./docs/fixtures/agent-decision-client-adopter-summary.schema.json)，通过态和失败态样例见 [passed.json](./docs/fixtures/agent-decision-client-adopter-summary/passed.json) 与 [invalid-response.json](./docs/fixtures/agent-decision-client-adopter-summary/invalid-response.json)。
...
机器断言推荐运行 `scripts/showcase-agent-decision-client-ci.sh --json`，直接读取 `schema_version`、`status`、`fixture_count`、`decisions[]`、`failures[]`、`result_json`、`result_schema` 和 `validator_exit_code`；summary 结构契约见 [agent-decision-client-ci-summary.schema.json](./docs/fixtures/agent-decision-client-ci-summary.schema.json)，通过态样例见 [passed.json](./docs/fixtures/agent-decision-client-ci-summary/passed.json)。客户端不想引入 JSON Schema 工具链时，可以运行 `node scripts/validate-agent-decision-client-ci-summary.mjs /path/to/testloop-agent-decision-client-summary.json` 做无依赖校验。如果要把该 summary 转成 Agent 下一步动作，可运行 `node scripts/render-agent-decision-client-ci-response.mjs /path/to/testloop-agent-decision-client-summary.json`，通过态输出 `agent_next_step=ready`；response 结构契约见 [agent-decision-client-ci-response.schema.json](./docs/fixtures/agent-decision-client-ci-response.schema.json)，通过态样例见 [passed.json](./docs/fixtures/agent-decision-client-ci-response/passed.json)，无依赖校验入口是 `node scripts/validate-agent-decision-client-ci-response.mjs /path/to/client-ci-response.json`。安装到外部仓库的 CI 模板会上传 `testloop-agent-decision-client-response-validation.json`，用于固定 response artifact 的消费结果。
...
也可以不 clone 仓库，直接下载 v0.5.21 的单脚本安装模板：
...
它会临时创建外部 client，生成 workflow，运行 helper dry-run，并校验安装 summary、基础客户端 CI summary、基础客户端 CI response、导出的 fixture manifest 和 `agent-decision-fixtures-result.json`，同时生成可直接交给 Agent 的 `agent_response_json`。
...
安装 summary 可用 `node scripts/validate-agent-decision-release-response-client-install-summary.mjs /path/to/install-summary.json` 校验，结构契约见 `agent-decision-release-response-client-install-summary.schema.json`。
如果要模拟接入方仓库的 GitHub Actions 形态，可运行 `scripts/showcase-agent-decision-client-release-response-ci.sh --json`。
如果要看外部仓库可直接照抄的最小样板，见 [Release response 接入方样板](./examples/release-response-adopter/README.md)，也可以运行 `scripts/showcase-release-response-adopter.sh --json` 验证 installer、workflow、`npm test --silent` 和接入方消费 helper 的完整链路；该脚本会默认生成 `testloop-release-response-adopter-artifacts/`，可直接作为 CI artifact 上传。summary 可用 `node scripts/validate-release-response-adopter-summary.mjs /path/to/release-response-adopter-summary.json` 校验，artifact 下载后可用 `node scripts/verify-release-response-adopter-artifact.mjs /path/to/testloop-release-response-adopter-artifacts` 离线自检，也可用 `node scripts/validate-release-response-adopter-artifact-verification.mjs /path/to/testloop-release-response-adopter-artifact-verification.json` 校验 verifier JSON。结构契约见 `release-response-adopter-summary.schema.json` 和 `release-response-adopter-artifact-verification.schema.json`，失败态样例见 `invalid-response.json` 和 `missing-summary-consumer.json`。如果要看客户端如何消费 artifact verification JSON，可运行 `go run ./examples/release-response-adopter-artifact-demo docs/fixtures/release-response-adopter-artifact-verification/passed.json`；通过态输出 `client_decision=accept`，缺文件失败态输出 `client_decision=inspect-artifact`。validator 失败时可用 `--json` 读取 `agent_next_step` 和 `failures[]`，也可以用 `read-testloop-release-response-summary.mjs` 输出 `testloop_release_response_summary_next_step`。
