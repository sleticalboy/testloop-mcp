...
当前 `v0.5.18` Release 已提供 Linux amd64、Linux arm64、macOS arm64、Windows amd64 和 Windows arm64 二进制。手动下载示例：
...
```bash
TESTLOOP_REPORT_EXPECT_VERSION=0.5.18 \
...
也可以不 clone 仓库，直接下载 v0.5.18 的单脚本安装模板：
...
```bash
scripts/verify-release-candidate.sh v0.5.18
```
