...
func TestGenerateTestsWithProviderIncludesResolvedStarBarrelImportedTypePayload(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell script fake provider is unix-only")
	}
...
`)
	...
	assertProviderSliceContains(t, target.PayloadNotes, "return annotation imported type ExternalUser from './models' resolved from models/user.ts")
	if !strings.Contains(req.StaticCode, "json: async () => ({ userId: 1, email: 'user@example.com' })") ||
		!strings.Contains(req.StaticCode, "expect(result).toEqual({ userId: 1, email: 'user@example.com' });") {
		t.Fatalf("provider static_code missing star barrel imported type payload:\n%s", req.StaticCode)
	}
}
