...
func TestHandleFixSuggestionsUsesParsedFrameworkFixtures(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "node-test assertion error",
			framework: "node-test",
			fixture:   "node_test_failure.txt",
			sourceRel: "sum.js",
			sourceLines: []string{
			...
			testRel: filepath.Join("test", "sum.test.js"),
			testLines: []string{
			...
			wantCategory:    "expectation_mismatch",
			wantContextFile: filepath.Join("test", "sum.test.js"),
			wantContextLine: 6,
			wantTexts:       []string{"实际值: 4", "期望值: 3", "测试附近行", "assert.equal(add(1, 2), 3)"},
		},
		{
