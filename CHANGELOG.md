...
- `run_tests coverage=true` 会为 Go 写入 `testloop-cover.out`；`validate_coverage_task coverage=true` 支持读取该 coverprofile 校验目标行命中，测试通过但目标行仍未覆盖时返回 `failed/needs_better_input`。
...
- 新增 Agent 决策客户端 CI 模板安装 dry-run 通过态 JSON 样例，方便客户端做 golden test。
...
- MCP server implementation version 更新为 `0.5.16`。
...
- Release Artifacts、Post-Release Verify、Windows ARM64 Probe 和 Homebrew Formula 生成器已同步 `--help` 退出码 0，避免正式发布时仍按旧退出码 2 校验。
