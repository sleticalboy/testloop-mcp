...
- 新增 `scripts/validate-js-regression-samples.sh`、`scripts/validate-py-regression-samples.sh` 和 `scripts/validate-regression-smoke.sh`，把 ip2region Jest ready、仓库内 TypeScript no-runtime fixture 与 Click pytest ready 样本纳入固定 smoke 矩阵，并串联 Java + JS + Python 小回归。
- 新增 `docs/regression-smoke.md`，记录固定 smoke 的默认项目路径、JSONL 依赖、跳过开关、runner 约束和 JS no-runtime 样本边界。
- 新增仓库内 `testdata/js-no-runtime` fixture 与 `scripts/js-manual-review-runner.js`，让 JS regression smoke 可稳定覆盖 `manual_review_no_runtime`，不再依赖已漂移的外部 TS 项目样本。
