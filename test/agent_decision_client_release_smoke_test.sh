...
def validate_payload(payload, label):
    ...
    if payload.get("release_ref") != "v0.5.19":
        failures.append(f"{label} release_ref must be v0.5.19")
    installer_url = payload.get("installer_url")
    if label == "fixture sample":
        if installer_url != "https://raw.githubusercontent.com/sleticalboy/testloop-mcp/v0.5.19/scripts/install-agent-decision-client-ci-template.sh":
            failures.append(f"{label} installer_url must point at v0.5.19 raw installer")
    ...
    if payload.get("helper_refs") != {"install": "v0.5.19", "consumer": "v0.5.19"}:
