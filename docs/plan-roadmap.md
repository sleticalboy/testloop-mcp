...
1. [ ] 新增 `docs/fixtures/release-response-adopter-summary.schema.json`。
2. [ ] 新增通过态 fixture，固定 `status=passed`、`release_ref=v0.5.20`、`fixture_count=8`、`agent_next_step=ready`、`should_accept=true` 和 `npm_exit_code=0`。
3. [ ] 新增无依赖 validator，校验 showcase summary 的关键路径、状态和 Agent 分流字段。
4. [ ] README、客户端集成说明、release response 客户端文档和接入样板 README 同步 schema/validator 入口。
5. [ ] CI workflow 纳入 summary schema/validator 测试。
