...
已完成补充：外部客户端接入链路现在覆盖到“下载 installer 后生成 workflow 并执行 contract”的实际动作。下一步应跑安装 showcase、模板文档/YAML、客户端集成文档、CI workflow 自检、完整 shell 回归和 Go 测试；通过后提交并观察 main CI。
