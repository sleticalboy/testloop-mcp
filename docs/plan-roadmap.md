...
1. [ ] 新增 `docs/fixtures/release-response-adopter-summary/invalid-response.json` 或同类失败态 fixture。
2. [ ] 扩展 `scripts/validate-release-response-adopter-summary.mjs --json` 的失败输出文档，明确接入方如何读取 `failures[]`。
3. [ ] 扩展 schema/validator 测试，覆盖失败态 fixture 和动态坏 summary。
4. [ ] README、客户端集成说明、fixtures 索引和接入样板 README 同步失败态用法。
5. [ ] CI workflow 保持覆盖新增失败态测试。
