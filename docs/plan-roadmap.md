...
已完成补充：Java ready 的定义从“生成测试可运行”推进到“生成测试可运行且目标 line_range 被 JaCoCo 确认覆盖”。当前已覆盖真实命中样本 `Failable`、`ClassUtils`、`CharSequenceUtils`，也覆盖了需要降级为手审的 `Metaphone`。Java 真实项目回归现在可以按 task id 精确筛选，也可以复用已有 task JSONL 跳过 baseline coverage。下一步应继续收敛 Maven/JUnit 运行阶段耗时：优先让验证入口支持只跑生成的 TestLoop 测试类或测试方法，同时保留 JaCoCo 目标行命中校验。
