...
已完成补充：第十一个真实样本把 Python/pytest 链路推进到 ASGI/HTTP 服务型场景。Starlette 样本暴露的通用问题包括仓库根包 import、coverage 配置污染测试文件、ASGI scope 最小输入、认证 scope 对象、配置文件读取和类型转换，以及可变多值字典的状态化断言。当前 Starlette top30 已达到 `ready=30`、`skipped_total=0`。下一步建议继续扩大 Starlette 到 top50，重点观察 TestClient、headers/cookies、responses、middleware 和 background task；或者切换到带数据库/外部服务依赖的 FastAPI 项目，重点验证依赖注入、monkeypatch 和 `manual_review_external_service` 分类。
