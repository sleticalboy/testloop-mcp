...
已完成补充：Java ready 的定义从“生成测试可运行”推进到“生成测试可运行且目标 line_range 被 JaCoCo 确认覆盖”。当前已覆盖真实命中样本 `Failable`、`ClassUtils`、`CharSequenceUtils`，也覆盖了需要降级为手审的 `Metaphone`。Java 真实项目回归现在可以按 task id 精确筛选，下一步应优先收敛验证耗时：复用 baseline coverage task 清单或支持从已有 task JSONL 输入直接验证，减少每次回归都要重跑 baseline coverage 的成本。
