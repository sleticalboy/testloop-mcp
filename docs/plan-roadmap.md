...
1. [ ] 扩展 `examples/release-response-adopter/scripts/read-testloop-release-response.mjs` 或新增 summary reader，展示接入方如何消费 adopter summary 的 failed 输出。
2. [ ] 在 `examples/release-response-adopter/README.md` 增加失败态本地命令，明确 `should_accept=false` 时不要继续发布。
3. [ ] 增加回归测试，固定 failed summary -> Agent 动作输出，不依赖 validator 文本。
4. [ ] README、客户端集成说明和 fixtures 索引同步该消费路径。
5. [ ] 提交并观察 main CI。
