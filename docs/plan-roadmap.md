...
5. [x] README、coverage formats、工具 schema 和 changelog 已同步 Node TAP coverage report 支持。
6. [x] 本地已通过 `go test ./internal/coverage ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh` 和 `git diff --check`。
