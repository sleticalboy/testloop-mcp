...
已完成补充：第十二个真实样本把 Python/pytest 链路推进到带业务依赖的 FastAPI 后端。它暴露了无 pyproject/test config 的项目根推断问题，以及文件/zip 输入、原地写入 dict helper、外部 SDK fake 和对象存储依赖隔离这些真实服务中常见的生成质量缺口。当前 haoy-apk-station backend top20 已达到 `ready=20`、`skipped_total=0`，普通生成失败清零。下一步建议继续扩大该样本到 top40，重点观察数据库异常、对象存储失败回滚、下载重定向、短链访问、隐藏应用、统计聚合和外部服务依赖是否能稳定归类。
