...
1. [ ] 选择一个 RocketMQ 之外的 TypeScript 项目，优先覆盖 Vitest 或 Jest，并包含 imported interface、enum、可选 class 参数或异步 client mock。
2. [ ] 用现有 `validate-js-coverage-top-tasks.sh` 先跑可控文件级 top20，记录 `ready/manual_review/repair_generated_test` 分布。
3. [ ] 如果出现普通 `repair_generated_test`，优先归因到通用 JS/TS 生成能力缺口；只有确认为外部服务、私有方法、no-runtime 或集成环境依赖时，才进入 manual review action。
4. [ ] 将新增失败类型沉淀为 generator/parser 单元测试，再复跑真实项目样本确认指标改善。
