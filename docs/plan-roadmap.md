...
已完成补充：仓库内 regression fixture 现在基本不再依赖运行时 JSONL 生成；`fixture-task-jsonl.py` 仍保留给 mcp-hub 和 haoy-apk-station 这类外部真实项目样本，因为它们的项目根路径来自本机环境，不适合直接写入仓库 fixture。下一步运行本地 gate、提交并等待远端 CI。
