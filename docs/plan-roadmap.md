...
已完成补充：第五真实样本证明 JS/Jest 链路还需要覆盖 ESM Jest 的 `NODE_OPTIONS`、monorepo 子包外部测试资源、对象参数分支输入合成，以及未导出顶层 helper 的公共入口覆盖。`ip2region` top20 已达到 `ready=20`、`skipped_total=0`。下一步建议扩大到 `ip2region` top30，继续查找纯函数、Buffer、文件 I/O 和二进制解析相关的新失败类型；如果 top30 稳定，再切换到 TypeScript/Jest 项目样本。
