...
已完成补充：主 MCP 工具现在不再只依赖文本 JSON 承载结构化数据，Agent 可以优先读 `structuredContent`，旧消费方仍可继续读 `content[0].text`。这比继续堆“测试生成器”能力更贴合项目定位，因为它降低了 Codex/Claude/Cursor 类 Agent 在测试闭环中的解析成本和字段漂移风险。下一步建议继续强化结构化契约：补一组 e2e 层断言，确认通过真实 MCP session 调用时 `structuredContent` 会按 SDK 序列化为 JSON RawMessage；随后再回到 Rust 真实样本，或启动 Java top-task 验证入口。
