...
8. [ ] Post-Release Verify run `29593507242` 已触发；当前等待 GitHub runner 调度完成。
9. [ ] `brew fetch` 下载验证受 GitHub/Homebrew 网络队列影响未完成；后续网络稳定后补跑安装级验证。

已完成补充：v0.5.1 已正式发布，版本重点是 MCP 客户端接入契约、真实 fixture、结构化分流和 release 文档入口的回归保护。Post-Release Verify 已触发但受 GitHub runner 队列影响尚未完成。下一步建议先回查 run `29593507242`，通过后再回到功能侧，优先围绕客户端可复制接入继续增强真实 e2e 示例和安装后自检体验。
