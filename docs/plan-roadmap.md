...
已完成补充：接入方现在不仅能看到 release response 的 ready 样例，也能固定 installer、基础客户端、consumer 和 fixture 漂移四类失败分流。下一步应跑 fixture/docs/CI/Go 门禁、提交并等待 main CI；CI 通过后继续评估真实第三方客户端模板是否仍有必要。
