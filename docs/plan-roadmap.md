...
8. [x] `cd44986` 远端 CI run `29893990689` passed，覆盖 artifact verification 客户端消费 demo、server build、testgen build 和 Docker image build。
9. [x] `61e4e19` 远端 CI run `29894128846` passed，覆盖 artifact 消费 demo CI 记录、server build、testgen build 和 Docker image build。
10. [x] `91c3498` 远端 CI run `29894309974` passed，覆盖 artifact 消费 demo 记录、server build、testgen build 和 Docker image build。
11. [x] `5b197ed` 远端 CI run `29894452504` passed，完成 artifact 消费 demo 记录的再验证，覆盖 server build、testgen build 和 Docker image build。

当前目标：让接入方不仅能校验 artifact verification JSON，还能看到最小客户端如何把结果映射成 Agent/CI 决策。

## 第四百六十二阶段：v0.5.21 候选发布边界整理

1. [x] 新增 `docs/plan-release-notes-v0.5.21.md`，归纳 v0.5.20 之后 release response 接入样板、artifact 打包、离线自检、JSON 契约和客户端消费 demo 的候选发布说明。
2. [x] 新增 `docs/plan-release-v0.5.21.md`，整理 v0.5.21 发布检查清单、已验证项、发布前门禁和正式发布待办。
3. [x] `CHANGELOG.md` 的 `Unreleased` 已记录 v0.5.21 候选发布文档。
4. [x] `6c2b781` 远端 CI run `29894713665` passed，覆盖 artifact 消费 demo 再验证记录、server build、testgen build 和 Docker image build。
5. [x] `d89ded8` 远端 CI run `29894889687` passed，覆盖 v0.5.21 候选发布边界整理、server build、testgen build 和 Docker image build。

当前目标：把 v0.5.21 从“功能持续堆叠”收敛到可发布候选边界，下一步先跑候选 release readiness，再决定是否进入正式版本准备。

## 第四百六十三阶段：v0.5.21 正式版本准备

1. [x] `main.go` MCP implementation version 已更新到 `0.5.21`。
2. [x] `CHANGELOG.md` 已将 Unreleased 内容收敛为 `v0.5.21 - 2026-07-22`，并保留新的空 Unreleased。
3. [x] README、installation、quickstart、first-run/onboarding/verification CI、Agent 决策客户端 CI 模板、release smoke、release response fixture、脚本默认版本和测试期望已同步到 `0.5.21` / `v0.5.21`。
4. [x] `docs/plan-release-v0.5.21.md` 和 `docs/plan-release-notes-v0.5.21.md` 已标记正式版本准备状态。
5. [x] 完整本地门禁已通过：`TESTLOOP_RELEASE_CANDIDATE_DIST_DIR=/tmp/testloop-v0.5.21-release-prep-dist scripts/verify-release-candidate.sh v0.5.21` 输出 `release_candidate_status=passed`，候选二进制 `--version` 输出 `testloop-mcp 0.5.21`。
6. [x] readiness 覆盖 release response adopter summary：`release_response_adopter_summary_status=passed release_ref=v0.5.21`。
7. [x] readiness 覆盖 artifact verification：`release_response_adopter_artifact_verification_status=passed release_ref=v0.5.21`。
8. [x] readiness 覆盖 darwin arm64 打包 dry-run 和 sha256 校验：`testloop-mcp_v0.5.21_darwin_arm64.tar.gz: OK`。

当前目标：提交并推送 v0.5.21 正式版本准备改动，等待 main CI；通过后进入是否打 tag、创建 GitHub Release 和更新 Homebrew tap 的决策点。

## 第四百六十四阶段：Node.js 内置 test runner 闭环

1. [x] `detector` 已能从 `package.json` 的 `scripts.test` 中识别 `node --test`，并回写框架名 `node-test`。
2. [x] `run_tests` 已支持 `framework=node-test`，从 `package.json` 所在目录执行 `node --test`，并把目标测试文件转成相对项目根路径。
3. [x] 新增 Node TAP parser，支持解析 `# tests/# pass/# fail` 汇总、`not ok` 失败、expected/actual、错误信息和堆栈位置。
4. [x] `generate_tests` 已支持 `framework=node-test`，生成 `node:test` / `node:assert/strict` 风格测试。
5. [x] README、Agent 工作流和结构化契约已同步 `node-test` 支持边界。
6. [x] 真实 Node `v20.20.1` smoke 已覆盖 `describe/it` 输出形态，并据此修复 suite 聚合 `not ok` 被误当失败用例、`error: |-` 没有提取真实断言消息的问题。
7. [x] 本地已通过 `go test ./internal/parser ./tools ./internal/generator ./internal/detector`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh` 和 `git diff --check`。
8. [x] `64e0194` 远端 CI run `29900303377` passed，覆盖 Node 内置 test runner 闭环、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百六十五阶段：Node.js 内置 coverage 回填

1. [x] 使用真实 Node `v20.20.1` 验证 `node --test --experimental-test-coverage` 会输出 TAP coverage report，并在 `# all files` 行提供 line coverage。
2. [x] `ParseNodeTest` 已解析 `# all files | line %`，回填 `coverage_percent`。
3. [x] `run_tests coverage=true` 的 `node-test` 回归已固定命令参数顺序：`node --test --experimental-test-coverage <relative-test-file>`。
4. [x] README 已说明 `node-test coverage=true` 的覆盖率来源和回填行为。
5. [x] 本地已通过 `go test ./internal/parser ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh` 和 `git diff --check`。
6. [x] `3f1e8d6` 远端 CI run `29900995052` passed，覆盖 Node coverage 回填、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百六十六阶段：Node TAP coverage report 任务化

1. [x] 新增 `ParseNodeTestCoverage`，支持解析 `node --test --experimental-test-coverage` 输出的 TAP coverage table。
2. [x] `parse_coverage framework=node-test` 已接入 Node TAP coverage report，返回 `framework=node-test`、`total_percent`、`files[]`、`suggestions[]` 和 `test_tasks[]`。
3. [x] `node-test` coverage task 已接入 JS/TS source map，能把未覆盖行映射到函数/方法目标；无法映射时仍保留文件级任务。
4. [x] coverage task 命令、推荐测试文件和测试名已支持 `node-test`：`node --test`、`*.test.js`、`covers <target> coverage gap`。
5. [x] 新增 handler 级闭环测试，固定 `parse_coverage framework=node-test -> generate_tests` 会生成 `node:test` / `node:assert/strict` 风格的 coverage task 测试草稿。
6. [x] README、coverage formats、工具 schema 和 changelog 已同步 Node TAP coverage report 支持。
7. [x] 本地已通过 `go test ./internal/coverage ./tools`、`go test ./tools -run 'TestHandleParseCoverageGenerateTestsNodeTestLoop|TestHandleParseCoverageParsesNodeTestCoverageReport' -count=1`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh` 和 `git diff --check`。
8. [x] `e70fb7b` 远端 CI run `29902014490` passed，覆盖 Node TAP coverage report 任务化 handler 闭环、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百六十七阶段：Node coverage task 验证闭环

1. [x] 新增 `validate_coverage_task` handler 回归，覆盖 `node-test` coverage task 的 `generate_tests -> run_tests` 闭环。
2. [x] 回归测试使用 fake `node` 固定生成后执行路径，确认验证结果返回 `status=passed`、`action=ready`、`run_result.framework=node-test`。
3. [x] 回归测试校验生成文件使用 `node:test` 和 `node:assert/strict`，并保留 coverage task 分支断言。
4. [x] 本地已通过 `go test ./tools -run TestHandleValidateCoverageTaskGeneratesAndRunsNodeTestTask -count=1`、`go test ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh` 和 `git diff --check`。
5. [x] `99192c9` 远端 CI run `29902791661` passed，覆盖 Node coverage task 验证闭环、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百六十八阶段：Node coverage task 目标行命中校验

1. [x] `validate_coverage_task coverage=true` 已支持从 `node-test` TAP coverage raw output 解析目标文件覆盖情况。
2. [x] 目标行仍在 TAP coverage 未覆盖列表中时，验证结果会从 `passed/ready` 降级为 `failed/needs_better_input`。
3. [x] metadata 已回填 `coverage_target_hit=false`、`coverage_report=node-test raw_output`、`coverage_hit_lines`、`coverage_missed_lines` 和 `coverage_miss_reason`，供 Agent 重新选择输入或公共入口。
4. [x] 新增 handler 回归同时固定命中与未命中两条路径：命中时保持 `passed/ready`，未命中时必须返回 `failed/needs_better_input`。
5. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/agent_decision_release_response_checklist_doc_test.sh`、`sh test/fixtures_index_test.sh` 和 `git diff --check`。
6. [x] `b4f3ea8` 远端 CI run `29903547456` passed，覆盖 Node coverage target-hit 校验、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百六十九阶段：pytest coverage task 目标行命中校验

1. [x] `ParsePytestCoverage` 已支持直接接收 `coverage.json` 文件路径，和 Go coverprofile、JaCoCo XML 的解析入口保持一致。
2. [x] `validate_coverage_task coverage=true` 已支持从 pytest 项目根目录 `coverage.json` 解析目标文件覆盖情况。
3. [x] 目标行仍在 coverage.py `missing_lines` 中时，验证结果会从 `passed/ready` 降级为 `failed/needs_better_input`。
4. [x] 新增 handler 回归同时固定 pytest 命中与未命中两条路径：命中时返回 `coverage_target_hit=true`，未命中时返回 `failed/needs_better_input` 并指向 `coverage.json`。
5. [x] 本地已通过 `go test ./internal/coverage ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/fixtures_index_test.sh`、`sh test/agent_decision_release_response_checklist_doc_test.sh` 和 `git diff --check`。
6. [x] `9d1945f` 远端 CI run `29904550822` passed，覆盖 pytest coverage target-hit 校验、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十阶段：Istanbul coverage task 目标行命中校验

1. [x] `ParseJestCoverage` 已支持直接接收 `coverage/coverage-final.json` 文件路径，和 coverage.py JSON / JaCoCo XML 的验证入口保持一致。
2. [x] `validate_coverage_task coverage=true` 已支持从 Jest/Vitest/Mocha 项目根目录 `coverage/coverage-final.json` 解析目标文件覆盖情况。
3. [x] 目标行对应 Istanbul statement hit count 为 0 时，验证结果会从 `passed/ready` 降级为 `failed/needs_better_input`。
4. [x] 新增 Vitest handler 回归同时固定命中与未命中两条路径：命中时返回 `coverage_target_hit=true`，未命中时返回 `failed/needs_better_input` 并指向 `coverage/coverage-final.json`。
5. [x] 本地已通过 `go test ./internal/coverage ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/fixtures_index_test.sh`、`sh test/agent_decision_release_response_checklist_doc_test.sh` 和 `git diff --check`。
6. [x] `a9a25d2` 远端 CI run `29905356462` passed，覆盖 Istanbul coverage target-hit 校验、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十一阶段：Rust LCOV coverage task 目标行命中校验

1. [x] `validate_coverage_task coverage=true` 已支持从 Rust/Cargo LCOV 文件解析目标文件覆盖情况。
2. [x] 默认读取 `target/tarpaulin/lcov.info`，并支持 `TESTLOOP_VALIDATE_RUST_COVERAGE_FILE` 指向 `cargo llvm-cov` 或项目自定义 LCOV。
3. [x] 目标行对应 LCOV `DA` hit count 为 0 时，验证结果会从 `passed/ready` 降级为 `failed/needs_better_input`。
4. [x] 新增 Rust handler 回归同时固定命中与未命中两条路径：命中时返回 `coverage_target_hit=true`，未命中时返回 `failed/needs_better_input` 并指向 `target/tarpaulin/lcov.info`。
5. [x] 本地已通过 `go test ./internal/coverage ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/fixtures_index_test.sh`、`sh test/agent_decision_release_response_checklist_doc_test.sh` 和 `git diff --check`。
6. [x] `64eb2d1` 远端 CI run `29906157326` passed，覆盖 Rust LCOV target-hit 校验、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十二阶段：Go coverprofile coverage task 目标行命中校验

1. [x] `run_tests coverage=true` 已为 Go 追加 `-coverprofile <module>/testloop-cover.out`，同时保留原有测试输出中的 `coverage_percent` 回填。
2. [x] `validate_coverage_task coverage=true` 已支持读取 Go coverprofile 并按 `coverage_task.line_range` 校验目标行命中。
3. [x] 目标行对应 coverprofile block count 为 0 时，验证结果会从 `passed/ready` 降级为 `failed/needs_better_input`。
4. [x] 新增 Go handler 回归同时固定命中与未命中两条路径：命中时返回 `coverage_target_hit=true`，未命中时返回 `failed/needs_better_input` 并指向 `testloop-cover.out`。
5. [x] `goTestCommand` 回归已固定 `coverage=true` 时的 `-coverprofile` 参数。
6. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/fixtures_index_test.sh`、`sh test/agent_decision_release_response_checklist_doc_test.sh` 和 `git diff --check`。
7. [x] `d2effda` 远端 CI run `29908207514` passed，覆盖 Go coverprofile target-hit 校验、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十三阶段：coverage target-hit 支持矩阵契约

1. [x] `coverageTaskTargetLineHit` 已从内联多条件判断收敛到 `coverageTaskTargetLineHitSupported`，避免支持框架列表继续散落在业务分支里。
2. [x] 新增 `coverageTaskTargetLineHitFrameworks` 作为当前 target-hit 支持矩阵，明确固定 Go coverprofile、Rust LCOV、Istanbul、Node TAP raw output、pytest coverage.json 和 Java JaCoCo 六类覆盖率来源对应的 8 个框架标签。
3. [x] 新增 `TestCoverageTaskTargetLineHitSupportedFrameworkMatrix`，同时固定支持列表、大小写/空白归一化和未支持框架拒绝路径。
4. [x] 本地已通过 target-hit 相关 handler 回归：Go、Node、pytest、Vitest、Rust 和 Java line miss 代表路径均保持通过。
5. [x] `5da7eda` 远端 CI run `29908700639` passed，覆盖矩阵契约回归、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十四阶段：Go coverprofile 路径配置可发现性

1. [x] 新增 `TestGoCoverageProfilePathHonorsEnvironment`，固定 `TESTLOOP_GO_COVERPROFILE` 相对路径按 module root 解析、绝对路径保持不变。
2. [x] README 的 `run_tests coverage=true` 说明已补充 `TESTLOOP_GO_COVERPROFILE`，避免自定义 coverprofile 路径只藏在代码里。
3. [x] README、Agent Contract 和 Agent Action 决策表均已说明 `validate_coverage_task coverage=true` 的 Go 默认读取 `testloop-cover.out`，并支持 `TESTLOOP_GO_COVERPROFILE` 指向相对项目根或绝对路径。
4. [x] `5714322` 远端 CI run `29909213067` passed，覆盖 Go coverprofile 路径配置回归、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十五阶段：Go 真实项目验证启用目标行命中

1. [x] `TestValidateGoCoverageTopTasks` 调用 `validate_coverage_task` 时已传入 `Coverage: true`，真实项目 opt-in 验证从“生成测试可运行”升级为“生成测试可运行且目标 line_range 被 coverprofile 确认命中”。
2. [x] `scripts/validate-go-coverage-top-tasks.sh` 新增 `TESTLOOP_VALIDATE_GO_COVERPROFILE`，并转发为 `TESTLOOP_GO_COVERPROFILE`，用于控制每个隔离 task worktree 的 coverprofile 路径。
3. [x] README 的真实项目验证脚本说明已补充 Go 验证阶段的 `coverage=true` 行为和 `TESTLOOP_VALIDATE_GO_COVERPROFILE` 用法。
4. [x] `960baaa` 远端 CI run `29909678735` passed，覆盖 Go 真实项目目标行命中验证、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十六阶段：跨语言真实项目验证启用 coverage target-hit

1. [x] JS/TS opt-in 真实项目验证调用 `validate_coverage_task` 时已传入 `Coverage: true`，让 Jest/Vitest/Mocha 的 Istanbul `coverage-final.json` 参与目标行命中判断。
2. [x] Python opt-in 真实项目验证调用 `validate_coverage_task` 时已传入 `Coverage: true`，让 pytest `coverage.json` 参与目标行命中判断。
3. [x] Rust opt-in 真实项目验证调用 `validate_coverage_task` 时已传入 `Coverage: true`，让 Rust LCOV 参与目标行命中判断。
4. [x] 新增 `TestProjectValidationEntrypointsEnableCoverageTargetHit`，固定 Go、JS/TS、Python、Rust 真实项目验证入口必须显式开启 coverage target-hit；Java/JUnit 继续沿用现有自动 JaCoCo report 路径。
5. [x] `d536a90` 远端 CI run `29910112421` passed，覆盖跨语言真实项目 target-hit 入口契约、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十七阶段：Rust per-task coverage 命令复用自定义 LCOV 配置

1. [x] `collectRustCoveragePercent` 已从硬编码 tarpaulin 改为复用 `rustCoverageCommand`，支持 `TESTLOOP_VALIDATE_RUST_COVERAGE_COMMAND` 指向 `cargo llvm-cov` 或项目自定义 coverage 命令。
2. [x] 新增 `rustCoverageFilePath`，统一 baseline、`run_tests coverage=true` 和 `validate_coverage_task coverage=true` 的 LCOV 文件路径解析逻辑。
3. [x] `rustCoverageReportForValidation` 已复用 `rustCoverageFilePath`，避免 per-task target-hit 读取路径与 coverage 生成路径不一致。
4. [x] 新增 `TestRustCoverageFilePathHonorsEnvironment`，固定默认 tarpaulin LCOV、相对自定义 LCOV 和绝对自定义 LCOV 三条路径。
5. [x] README 已说明 Rust baseline 和每个 task 验证阶段都会共用 `TESTLOOP_VALIDATE_RUST_COVERAGE_COMMAND` / `TESTLOOP_VALIDATE_RUST_COVERAGE_FILE`。
6. [x] `2bb03f4` 远端 CI run `29910622610` passed，覆盖 Rust per-task coverage 配置复用、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十八阶段：JS/Python coverage report 路径配置复用

1. [x] 新增 `jsCoverageFilePath`，baseline 解析和 `validate_coverage_task coverage=true` 的 Istanbul target-hit 校验都支持 `TESTLOOP_VALIDATE_JS_COVERAGE_FILE`。
2. [x] 新增 `pyCoverageFilePath`，baseline 解析和 `validate_coverage_task coverage=true` 的 pytest target-hit 校验都支持 `TESTLOOP_VALIDATE_PY_COVERAGE_FILE`。
3. [x] 新增 `TestJSCoverageFilePathHonorsEnvironment` 和 `TestPyCoverageFilePathHonorsEnvironment`，固定默认路径、相对自定义路径和绝对自定义路径。
4. [x] `scripts/validate-js-coverage-top-tasks.sh --help` 和 `scripts/validate-py-coverage-top-tasks.sh --help` 已暴露自定义 coverage report 路径变量，并由脚本测试固定。
5. [x] README 已说明 JS/Python baseline 与 per-task target-hit 会读取同一份可配置 coverage report。
6. [x] `0d99fb9` 远端 CI run `29911428542` passed，覆盖 JS/Python coverage report 路径配置复用、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百七十九阶段：Java JaCoCo report 路径配置复用

1. [x] `javaCoverageFile` 已从 opt-in 测试 helper 下沉到生产代码，统一处理 Maven 默认、Gradle 默认和 `TESTLOOP_VALIDATE_JAVA_COVERAGE_FILE` 自定义 JaCoCo XML。
2. [x] `collectJavaCoveragePercent` 已复用 `javaCoverageFile`，让 `run_tests coverage=true` 读取自定义 JaCoCo XML。
3. [x] `javaCoverageReportForValidation` 已复用 `javaCoverageFile`，让 `validate_coverage_task coverage=true` 的目标行命中校验读取同一份 JaCoCo XML。
4. [x] 新增 `TestJavaCoverageFileHonorsEnvironment`，固定默认 Maven 路径、Gradle fallback、相对自定义路径和绝对自定义路径。
5. [x] README 和脚本帮助测试已固定 `TESTLOOP_VALIDATE_JAVA_COVERAGE_FILE` 的可发现性。
6. [x] `f3362b8` 远端 CI run `29911920598` passed，覆盖 Java JaCoCo report 路径配置复用、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十阶段：Agent 文档同步 coverage report 配置矩阵

1. [x] `docs/agent-contract.md` 已补充 JS、Python、Rust、Java 自定义 coverage report 路径变量，避免只暴露 Go 的 `TESTLOOP_GO_COVERPROFILE`。
2. [x] `docs/agent-action-guide.md` 已同步相同配置矩阵，让客户端在处理 `coverage_target_hit=false` 时能知道各语言报告路径可配置。
3. [x] 配置矩阵当前覆盖：Go `TESTLOOP_GO_COVERPROFILE`、JS `TESTLOOP_VALIDATE_JS_COVERAGE_FILE`、Python `TESTLOOP_VALIDATE_PY_COVERAGE_FILE`、Rust `TESTLOOP_VALIDATE_RUST_COVERAGE_FILE`、Java `TESTLOOP_VALIDATE_JAVA_COVERAGE_FILE`。
4. [x] `f98fc7c` 远端 CI run `29912262217` passed，覆盖 Agent 文档 coverage report 配置矩阵同步、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十一阶段：coverage target-hit 支持矩阵结构化输出

1. [x] `validate_coverage_task coverage=true` 的 `metadata` 已新增 `coverage_target_hit_supported`，明确当前框架是否支持目标行命中校验。
2. [x] `metadata.coverage_target_hit_supported_frameworks` 已返回当前 target-hit 支持矩阵，避免客户端在不支持框架或缺少报告时只能靠缺失字段猜测。
3. [x] 新增 `TestCoverageTaskValidationMetadataExposesTargetHitSupportWhenCoverageRequested`，固定支持框架、不支持框架和 `coverage=false` 三条 metadata 契约。
4. [x] README、Agent Contract 和 Agent Action 决策表已同步新 metadata 字段的消费方式。
5. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/fixtures_index_test.sh` 和 `git diff --check`。
6. [x] `d4a6407` 远端 CI run `29912855342` passed，覆盖 coverage target-hit 支持矩阵结构化输出、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十二阶段：测试框架名入口归一化

1. [x] 新增 `normalizeFrameworkName`，统一处理测试框架名的空白和大小写。
2. [x] `run_tests` 已在入口归一化显式传入和自动检测的 framework，避免 ` GO-TEST `、`Vitest` 这类输入落入 unsupported 分支。
3. [x] `generate_tests` 已在入口归一化 framework，让静态生成和上下文构建拿到稳定框架名。
4. [x] `validate_coverage_task` 已在入口、metadata、target-hit 支持判断和 JUnit coverage 分支复用同一归一化逻辑。
5. [x] 新增 `TestNormalizeFrameworkName`、`TestHandleRunTestsNormalizesFrameworkInput`，并扩展 coverage metadata 测试固定规范化输出。
6. [x] 本地已通过 `go test ./tools`、`go test ./...` 和 `git diff --check`。
7. [x] `210cfbd` 远端 CI run `29913334394` passed，覆盖测试框架名入口归一化、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十三阶段：解析入口框架名归一化

1. [x] `internal/parser.ParseTestOutput` 已归一化 framework，包内直接调用也支持大小写和空白容错。
2. [x] `internal/coverage.ParseCoverage` 已归一化 framework，避免 ` GO-TEST ` 这类输入被误判为不支持的覆盖率框架。
3. [x] `parse_results` 和兼容函数 `parseResults` 已复用 `normalizeFrameworkName`，结构化返回中的 `framework` 稳定为规范值。
4. [x] `parse_coverage` 已复用 `normalizeFrameworkName`，MCP handler 和内部 coverage dispatcher 的行为一致。
5. [x] 新增 parser、coverage、`parse_results` handler 和 `parse_coverage` handler 的归一化回归。
6. [x] 本地已通过 `go test ./internal/parser ./internal/coverage ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
7. [x] `792be37` 远端 CI run `29913668459` passed，覆盖解析入口框架名归一化、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十四阶段：coverage task framework 输出归一化

1. [x] `coverageTaskForGeneration` 已接收入口 framework，并在返回的 coverage task 副本中规范化 `Framework`。
2. [x] `BuildGenerationContextWithOptions` 已统一计算 effective framework；有 coverage task 时也会覆盖 `ctx.Framework`，避免 JS/TS 显式 `vitest` 被上下文默认值 `jest` 覆盖。
3. [x] `ctx.CoverageTask.Framework` 已随上下文一起归一化，LLM provider 和静态 provider 看到同一份规范化任务。
4. [x] 新增 `TestHandleGenerateTestsNormalizesCoverageTaskFramework`，固定 `generated.coverage_task.framework`、`context.framework` 和 `context.coverage_task.framework` 同步为 `vitest`。
5. [x] 本地已通过 `go test ./internal/generator ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
6. [x] `2075838` 远端 CI run `29914011298` passed，覆盖 coverage task framework 输出归一化、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十五阶段：repair command framework 归一化

1. [x] `runTestRepairCommands` 已在 helper 内部归一化 framework，包内直接调用也能处理大小写和空白。
2. [x] 新增 Vitest repair command 回归，固定 ` VITEST ` 会输出 `npx vitest run <test_file>`。
3. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
4. [x] `261c2e0` 远端 CI run `29914249894` passed，覆盖 repair command framework 归一化、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十六阶段：JS opt-in 验证 framework 归一化

1. [x] `TestValidateJSCoverageTopTasks` 已对 `TESTLOOP_VALIDATE_JS_FRAMEWORK` 复用 `normalizeFrameworkName`，避免环境变量大小写或空白导致误判 unsupported。
2. [x] `sanitizeJSValidationTaskID` 已复用 framework 归一化，确保合成 type-only task id 稳定。
3. [x] `jsValidationCoverageTaskCommand` 已复用 framework 归一化，避免 helper 直接调用时生成错误命令。
4. [x] 新增 `TestJSValidationHelpersNormalizeFramework`，固定 ` VITEST ` task id 和 ` MOCHA ` command。
5. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
6. [x] `727c9e3` 远端 CI run `29914683259` passed，覆盖 JS opt-in 验证 framework 归一化、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十七阶段：coverage task helper framework 归一化

1. [x] 新增 `normalizeCoverageFramework`，统一 internal/coverage 包内 coverage framework 的空白和大小写处理。
2. [x] `ParseCoverage`、`coverageSuggestionSupportsGeneratedTask`、`coverageTaskCommand`、`coverageTaskTestFile` 和 `coverageTaskTestName` 已复用同一归一化逻辑。
3. [x] 扩展 coverage task command/test file 回归，固定 ` VITEST `、` MOCHA ` 等输入仍生成正确命令和测试文件。
4. [x] 新增 `TestCoverageTaskTestNameNormalizesFramework`，固定 Go、pytest、Vitest 的测试名推荐不受 framework 大小写/空白影响。
5. [x] 本地已通过 `go test ./internal/coverage`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十八阶段：JS baseline coverage 命令 framework 归一化

1. [x] `jsCoverageCommand` 已在入口归一化 framework，默认 baseline coverage 命令支持大小写和空白容错。
2. [x] 新增 `TestJSCoverageCommandNormalizesFramework`，固定 ` VITEST ` 会生成 `npx vitest run --coverage <test>`。
3. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百八十九阶段：manual review 统一 metadata 字段

1. [x] `validate_coverage_task` 已在最终 action 确定后为 `manual_review_*` 结果补充 `metadata.manual_review_kind` 和 `metadata.manual_review_reason`。
2. [x] 原有分类字段如 `unreachable_reason`、`environment_reason`、`private_reason` 继续保留，保证旧客户端兼容。
3. [x] 新增 `TestCoverageTaskAnnotateActionMetadataAddsManualReviewReason`，固定 8 类 `manual_review_*` action 到统一 kind/reason 字段的映射。
4. [x] 扩展真实 unreachable handler 回归，确认 MCP 输出中包含统一 manual review metadata。
5. [x] Agent Action 决策表和 Agent Contract 已说明客户端优先读取统一字段，再回退分类字段。
6. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。

当前目标：提交并推送 manual review 统一 metadata 字段改动，等待 main CI；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十阶段：validate coverage task 通用下一步原因字段

1. [x] `validate_coverage_task` 已为 `needs_better_input` 结果补充 `metadata.next_action_kind=coverage_target_miss`、`metadata.next_action_reason` 和 `metadata.needs_better_input_reason`。
2. [x] `manual_review_*` 结果已同步补充通用 `metadata.next_action_kind=manual_review` 和 `metadata.next_action_reason`，同时保留 `manual_review_kind/manual_review_reason`。
3. [x] 新增 handler 级 Go 覆盖率 miss 回归，固定真实 MCP 输出中的通用下一步原因字段。
4. [x] 扩展 action metadata helper 回归，固定 `manual_review_*`、`needs_better_input` 和非原因类 action 的字段边界。
5. [x] Agent Action 决策表和 Agent Contract 已说明客户端优先读取 `next_action_reason`，再回退历史字段。
6. [x] 本地已通过 `go test ./tools`、`go test ./...`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
7. [x] `e797dbe` 已提交并推送，main CI run `29921584242` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十一阶段：needs_better_input 客户端消费契约同步

1. [x] `docs/fixtures/validate-coverage-task-needs-better-input.json` 已补充 `next_action_kind`、`next_action_reason` 和 `needs_better_input_reason`。
2. [x] `scripts/validate-agent-decision-fixtures.mjs` 已要求 `needs_better_input` fixture 暴露统一下一步原因字段，并校验与旧 `coverage_miss_reason` 一致。
3. [x] README、客户端集成说明、MCP 客户端契约测试说明、fixture 索引和结构化样例已改为优先读取 `metadata.next_action_reason`。
4. [x] 旧字段 `coverage_miss_reason` 仍保留并在文档中标为兼容 fallback。
5. [x] 本地已通过 `go test ./...`、`node scripts/validate-agent-decision-fixtures.mjs docs/fixtures/agent-decision-fixtures.json .`、`sh test/agent_decision_fixtures_manifest_test.sh`、`sh test/fixture_decision_mapping_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
6. [ ] 提交、推送和 main CI 待执行。

当前目标：完成 fixture/client 契约同步验证并提交推送；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十二阶段：Agent 决策示例输出分流原因

1. [x] `examples/agent-decision-demo` 已从 `metadata.next_action_reason` 及兼容原因字段中提取 reason，并在有原因时随决策行输出。
2. [x] `scripts/validate-agent-decision-fixtures.mjs --json` 已在 `fixtures[]` 中输出可用的 `reason`，方便客户端 CI artifact 直接交给 Agent 消费。
3. [x] 真实项目 `manual_review_environment` / `manual_review_external_service` 摘要已补充统一 reason 字段，避免 demo 只给动作不给原因。
4. [x] 新增 demo 与 validator 回归断言，固定 `manual_review_internal`、`manual_review_environment` 和 `needs_better_input` 三类原因输出。
5. [x] 客户端集成说明已补充 demo/validator JSON 的 reason 字段消费方式。
6. [x] 本地已通过 `sh test/agent_decision_demo_test.sh`、`sh test/agent_decision_fixture_validator_test.sh`、`sh test/agent_decision_fixtures_manifest_test.sh`、`sh test/fixture_decision_mapping_test.sh`、`go test ./...`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh` 和 `git diff --check`。
7. [x] `c605d74` 已提交并推送，main CI run `29922956992` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十三阶段：Agent 决策 fixture validator JSON 契约化

1. [x] `scripts/validate-agent-decision-fixtures.mjs --json` 已增加 `schema_version=1`，让输出可按版本演进。
2. [x] 新增 `docs/fixtures/agent-decision-fixtures-result.schema.json`，固定 validator JSON 的顶层字段、fixture result 字段和可选 `reason` 字段。
3. [x] 新增 `docs/fixtures/agent-decision-fixtures-result/passed.json`，作为当前 8 个决策 fixture 的通过态样例。
4. [x] 新增 `test/agent_decision_fixtures_result_schema_test.sh`，校验生成输出、schema 和 passed fixture 样例三者一致。
5. [x] 客户端集成说明和 fixture 索引已链接 validator result schema 与通过态样例。
6. [x] 本地已通过 `sh test/agent_decision_fixtures_result_schema_test.sh`、`sh test/agent_decision_fixture_validator_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`go test ./...`、`sh test/release_doc_index_test.sh`、`node --check scripts/validate-agent-decision-fixtures.mjs`、`sh test/ci_workflow_test.sh` 和 `git diff --check`。
7. [x] `62bacc9` 与修复提交 `a4019b2` 已推送；`62bacc9` 的 CI 因新增脚本未加入 workflow 失败，`a4019b2` 已补齐 workflow，main CI run `29923695456` passed。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十四阶段：Agent 决策 fixture 导出包携带 result 契约

1. [x] `scripts/export-agent-decision-fixtures.mjs` 已复制 `agent-decision-fixtures-result.schema.json` 和通过态 result fixture。
2. [x] 导出包 README 已说明 validator JSON result contract 的位置。
3. [x] `test/agent_decision_fixture_export_test.sh` 已断言导出包包含 result schema/passed fixture，并校验 `schema_version=1`。
4. [x] README 和客户端集成说明已同步导出包携带 validator result schema 的说明。
5. [x] 本地已通过 `sh test/agent_decision_fixture_export_test.sh`、`sh test/agent_decision_fixtures_result_schema_test.sh`、`sh test/agent_decision_fixture_validator_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`go test ./...`、`node --check scripts/export-agent-decision-fixtures.mjs`、`node --check scripts/validate-agent-decision-fixtures.mjs` 和 `git diff --check`。
6. [x] `a85e209` 已提交并推送，main CI run `29924132214` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十五阶段：基础客户端 CI summary 暴露 result schema

1. [x] `scripts/showcase-agent-decision-client-ci.sh --json` 已输出 `result_schema`，指向导出包内的 `agent-decision-fixtures-result.schema.json`。
2. [x] 文本输出已增加 `agent_decision_result_schema=...`，方便 shell/CI 日志直接定位 schema。
3. [x] `scripts/render-agent-decision-client-ci-response.mjs` 已把 `result_schema` 纳入 evidence，旧 summary 缺失该字段时保持兼容。
4. [x] showcase 和 response 回归测试已断言 `result_schema` 存在并指向真实 schema 文件。
5. [x] README 和客户端集成说明已同步基础客户端 CI summary 的 `result_schema` 字段。
6. [x] 本地已通过 `sh test/agent_decision_client_ci_showcase_test.sh`、`sh test/agent_decision_client_ci_response_test.sh`、`bash -n scripts/showcase-agent-decision-client-ci.sh`、`node --check scripts/render-agent-decision-client-ci-response.mjs`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`go test ./...` 和 `git diff --check`。
7. [x] `90611e2` 已提交并推送，main CI run `29924617645` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十六阶段：基础客户端 CI summary 契约化

1. [x] `scripts/showcase-agent-decision-client-ci.sh --json` 已增加 `schema_version=1`，让基础客户端 CI summary 可按版本演进。
2. [x] 新增 `docs/fixtures/agent-decision-client-ci-summary.schema.json`，固定基础 summary 的顶层字段、路径字段、决策序列和 validator 退出码。
3. [x] 新增 `docs/fixtures/agent-decision-client-ci-summary/passed.json`，作为当前 8 个决策 fixture 的通过态客户端 CI summary 样例。
4. [x] 新增 `test/agent_decision_client_ci_summary_schema_test.sh`，校验生成 summary、schema 和 passed fixture 样例三者一致。
5. [x] `.github/workflows/ci.yml` 已纳入基础客户端 CI summary schema 回归。
6. [x] README、客户端集成说明和 fixture 索引已链接基础客户端 CI summary schema 与通过态样例。
7. [x] 本地已通过 `sh test/agent_decision_client_ci_summary_schema_test.sh`、`sh test/agent_decision_client_ci_showcase_test.sh`、`sh test/agent_decision_client_ci_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`go test ./...`、`bash -n scripts/showcase-agent-decision-client-ci.sh`、`node --check scripts/render-agent-decision-client-ci-response.mjs` 和 `git diff --check`。
8. [x] `e7d7ee2` 已提交并推送，main CI run `29925391189` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十七阶段：基础客户端 CI summary 无依赖校验器

1. [x] 新增 `scripts/validate-agent-decision-client-ci-summary.mjs`，默认校验基础客户端 CI summary passed fixture，也可校验运行时生成的 summary。
2. [x] validator 支持 `--json`，失败时输出结构化 `failures[]`，方便客户端 CI 或 Agent 读取。
3. [x] 新增 `test/agent_decision_client_ci_summary_validator_test.sh`，覆盖默认通过态、JSON 模式、运行时 summary 和失败态 summary。
4. [x] `.github/workflows/ci.yml`、showcase 脚本检查、README、客户端集成说明和 fixture 索引已同步无依赖 validator 入口。
5. [x] 本地已通过 `sh test/agent_decision_client_ci_summary_validator_test.sh`、`sh test/agent_decision_client_ci_summary_schema_test.sh`、`sh test/agent_decision_client_ci_showcase_test.sh`、`sh test/agent_decision_client_ci_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`node --check scripts/validate-agent-decision-client-ci-summary.mjs` 和 `git diff --check`。
6. [x] `8c55847` 已提交并推送，main CI run `29926191750` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `ef449d5` 已提交并推送，main CI run `29926425593` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十八阶段：consumer smoke 复用基础 summary validator

1. [x] `scripts/showcase-agent-decision-client-consumer-smoke.sh` 已在读取基础客户端 CI summary 后运行 `validate-agent-decision-client-ci-summary.mjs --json`。
2. [x] consumer smoke summary 已新增 `client_summary_validator_json` 和 `client_summary_validator_exit_code`，通过态要求基础 summary validator 退出码为 0。
3. [x] consumer smoke summary schema、通过态 fixture 和失败态 fixture 已同步新增字段。
4. [x] consumer smoke、schema、summary validator、checklist 命令测试和相关文档已同步基础 summary validator 证据。
5. [x] 本地已通过 `sh test/agent_decision_client_ci_consumer_smoke_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_summary_schema_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_summary_validator_test.sh`、`sh test/agent_decision_client_ci_checklist_commands_test.sh`、`sh test/agent_decision_client_ci_summary_validator_test.sh`、`sh test/agent_decision_client_ci_checklist_doc_test.sh`、`sh test/agent_decision_client_ci_template_doc_test.sh`、`sh test/mcp_client_contract_doc_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`node --check scripts/validate-agent-decision-client-ci-summary.mjs`、`bash -n scripts/showcase-agent-decision-client-consumer-smoke.sh` 和 `git diff --check`。
6. [x] `181140c` 已提交并推送，main CI run `29927289635` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第四百九十九阶段：consumer response 识别基础 summary validator 失败

1. [x] `scripts/render-agent-decision-client-consumer-response.mjs` 已把 `client_summary_validator_json` 和四类 validator exit code 写入 evidence。
2. [x] renderer 已显式检查 `client_summary_validator_exit_code`，基础 summary validator 失败时分流到 `inspect-consumer-smoke-validator`。
3. [x] 新增 `docs/fixtures/agent-decision-client-consumer-smoke-summary/client-summary-validator-failed.json`，固定基础 summary validator 失败态样例。
4. [x] consumer response、consumer smoke schema/validator 和客户端接入文档测试已同步新增失败态 fixture。
5. [x] 本地已通过 `sh test/agent_decision_client_consumer_response_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_summary_schema_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_summary_validator_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/agent_decision_client_ci_checklist_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`node --check scripts/render-agent-decision-client-consumer-response.mjs` 和 `git diff --check`。
6. [x] `5d296d1` 已提交并推送，main CI run `29927795021` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百阶段：consumer response 结构契约化

1. [x] 新增 `docs/fixtures/agent-decision-client-consumer-response.schema.json`，固定 consumer response JSON 顶层字段和 evidence 字段。
2. [x] 新增 consumer response 通过态和失败态 fixture：`passed`、`client-summary-validator-failed`、`validator-failed`、`fixture-drift`。
3. [x] 新增 `test/agent_decision_client_consumer_response_fixtures_test.sh`，从 source summary 实时渲染 response，并与 fixture 对比。
4. [x] `.github/workflows/ci.yml`、fixture 索引和客户端集成说明已同步 consumer response schema/fixture 入口。
5. [x] 本地已通过 `sh test/agent_decision_client_consumer_response_fixtures_test.sh`、`sh test/agent_decision_client_consumer_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`node --check scripts/render-agent-decision-client-consumer-response.mjs` 和 `git diff --check`。
6. [x] `8395160` 已提交并推送，main CI run `29928377536` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零一阶段：consumer response 无依赖校验器

1. [x] 新增 `scripts/validate-agent-decision-client-consumer-response.mjs`，默认校验 consumer response passed fixture，也可校验运行时 response。
2. [x] validator 支持 `--json`，失败时输出结构化 `failures[]`、`agent_next_step` 和 evidence 决策摘要。
3. [x] 新增 `test/agent_decision_client_consumer_response_validator_test.sh`，覆盖默认通过态、JSON 模式、运行时 response 和失败态 response。
4. [x] `.github/workflows/ci.yml`、showcase 脚本检查、fixture 索引和客户端集成说明已同步无依赖 validator 入口。
5. [x] 本地已通过 `sh test/agent_decision_client_consumer_response_validator_test.sh`、`sh test/agent_decision_client_consumer_response_fixtures_test.sh`、`sh test/agent_decision_client_consumer_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/showcase_scripts_test.sh`、`sh test/docs_links_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/ci_workflow_test.sh`、`go test ./...`、`node --check scripts/validate-agent-decision-client-consumer-response.mjs` 和 `git diff --check`。
6. [x] `613c920` 已提交并推送，main CI run `29928951475` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零二阶段：基础客户端 CI response 结构契约化

1. [x] 新增 `docs/fixtures/agent-decision-client-ci-response.schema.json`，固定基础客户端 CI response JSON 顶层字段和 evidence 字段。
2. [x] 新增基础客户端 CI response 通过态和失败态 fixture：`passed`、`validator-failed`、`fixture-drift`。
3. [x] 新增 `test/agent_decision_client_ci_response_fixtures_test.sh`，从 source summary 实时渲染 response，并与 fixture 对比。
4. [x] `.github/workflows/ci.yml`、README、fixture 索引和客户端集成说明已同步基础客户端 CI response schema/fixture 入口。
5. [x] 本地已通过 `sh test/agent_decision_client_ci_response_fixtures_test.sh`、`sh test/agent_decision_client_ci_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/readme_ci_snippet_test.sh`、`go test ./...`、`node --check scripts/render-agent-decision-client-ci-response.mjs` 和 `git diff --check`。
6. [x] `6ff792b` 已提交并推送，main CI run `29930307642` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `c1a659c` 已提交并推送，main CI run `29930603151` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零三阶段：基础客户端 CI response 无依赖校验器

1. [x] 新增 `scripts/validate-agent-decision-client-ci-response.mjs`，默认校验基础客户端 CI response passed fixture，也可校验运行时 response。
2. [x] validator 支持 `--json`，失败时输出结构化 `failures[]`、`agent_next_step` 和 evidence 决策摘要。
3. [x] 新增 `test/agent_decision_client_ci_response_validator_test.sh`，覆盖默认通过态、JSON 模式、运行时 response 和失败态 response。
4. [x] `.github/workflows/ci.yml`、showcase 脚本检查、fixture 索引、README 和客户端集成说明已同步无依赖 validator 入口。
5. [x] 本地已通过 `sh test/agent_decision_client_ci_response_validator_test.sh`、`sh test/agent_decision_client_ci_response_fixtures_test.sh`、`sh test/agent_decision_client_ci_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/showcase_scripts_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`node --check scripts/validate-agent-decision-client-ci-response.mjs`、`sh test/docs_links_test.sh`、`sh test/readme_ci_snippet_test.sh`、`go test ./...` 和 `git diff --check`。
6. [x] `fb962e5` 已提交并推送，main CI run `29931084052` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `ca73780` 已提交并推送，main CI run `29931362624` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零四阶段：consumer smoke 接入基础 response validator

1. [x] `scripts/showcase-agent-decision-client-consumer-smoke.sh` 已生成并校验基础客户端 CI response，summary 新增 `client_response_json`、`client_response_validator_json` 和 `client_response_validator_exit_code`。
2. [x] consumer smoke summary schema、通过态 fixture 和失败态 fixture 已同步基础 response validator 证据。
3. [x] consumer smoke、summary schema 和 summary validator 测试已同步新增字段与路径存在性断言。
4. [x] README、客户端集成说明、fixture 索引、CI checklist 和模板文档已同步基础 response validator 证据。
5. [x] 本地已通过 `sh test/agent_decision_client_ci_consumer_smoke_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_summary_schema_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_summary_validator_test.sh`、`sh test/agent_decision_client_consumer_response_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/agent_decision_client_ci_checklist_doc_test.sh`、`sh test/agent_decision_client_ci_template_doc_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`bash -n scripts/showcase-agent-decision-client-consumer-smoke.sh`、`node --check scripts/validate-agent-decision-client-consumer-smoke-summary.mjs` 和 `git diff --check`。
6. [x] `d226f2c` 已提交并推送，main CI run `29932149140` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `ff81ed8` 已提交并推送，main CI run `29932414756` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零五阶段：客户端 CI 模板内置基础 response validator

1. [x] `scripts/install-agent-decision-client-ci-template.sh` 生成的 workflow 已新增 `Validate Agent decision response` step，运行 `validate-agent-decision-client-ci-response.mjs --json`。
2. [x] workflow artifact 清单已新增 `/tmp/testloop-agent-decision-client-response-validation.json`，用于接入方下载后直接消费 validator 结果。
3. [x] 模板缺失 summary 的 fallback response 已补齐 `result_schema` 字段，避免与基础 response schema 漂移。
4. [x] CI 模板文档、Checklist、README 和客户端集成说明已同步 response validation artifact。
5. [x] 本地已通过 `sh test/install_agent_decision_client_ci_template_test.sh`、`sh test/agent_decision_client_ci_template_yaml_test.sh`、`sh test/agent_decision_client_ci_template_doc_test.sh`、`sh test/agent_decision_client_ci_checklist_doc_test.sh`、`sh test/agent_decision_client_ci_template_dry_run_test.sh`、`sh test/agent_decision_client_ci_template_install_showcase_test.sh`、`sh test/agent_decision_client_ci_template_install_summary_schema_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/docs_links_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`bash -n scripts/install-agent-decision-client-ci-template.sh` 和 `git diff --check`。
6. [x] `c76f627` 已提交并推送，main CI run `29933092575` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `7bd7192` 已提交并推送，main CI run `29933372273` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零六阶段：安装 dry-run summary 固定 response validation 证据

1. [x] `scripts/showcase-agent-decision-client-ci-template-install.sh` 已在执行基础 contract 后渲染并校验基础客户端 CI response。
2. [x] 安装 dry-run summary schema、通过态 fixture 和无依赖 validator 已新增 `response_json`、`response_validation_json` 和 `response_validator_exit_code`。
3. [x] 安装 showcase、summary schema 和 install summary validator 测试已同步基础 response validation 证据。
4. [x] fixture 索引、客户端集成说明、CI checklist 和模板文档已同步安装 dry-run response validation 字段。
5. [x] 本地已通过 `sh test/agent_decision_client_ci_template_install_showcase_test.sh`、`sh test/agent_decision_client_ci_template_install_summary_schema_test.sh`、`sh test/agent_decision_client_ci_install_summary_validator_test.sh`、`sh test/agent_decision_client_ci_checklist_doc_test.sh`、`sh test/agent_decision_client_ci_template_doc_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...`、`bash -n scripts/showcase-agent-decision-client-ci-template-install.sh`、`node --check scripts/validate-agent-decision-client-ci-install-summary.mjs` 和 `git diff --check`。
6. [x] `38d44ad` 已提交并推送，main CI run `29933847830` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `f59d899` 已提交并推送，main CI run `29934088748` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零七阶段：consumer response 识别基础 response validator 证据

1. [x] `scripts/render-agent-decision-client-consumer-response.mjs` 已把 `client_response_json`、`client_response_validator_json` 和 `client_response_validator_exit_code` 写入 response evidence。
2. [x] consumer response schema、通过态 fixture 和失败态 fixture 已同步基础 response validator evidence。
3. [x] consumer response renderer 分流已调整优先级：fixture count/decision 漂移优先归到 `inspect-agent-decision-fixtures`，validator exit code 失败归到 `inspect-consumer-smoke-validator`。
4. [x] consumer response fixture、renderer、validator 和客户端集成文档测试已同步新增字段。
5. [x] 本地已通过 `sh test/agent_decision_client_consumer_response_test.sh`、`sh test/agent_decision_client_consumer_response_fixtures_test.sh`、`sh test/agent_decision_client_consumer_response_validator_test.sh`、`sh test/agent_decision_client_ci_consumer_smoke_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`node --check scripts/render-agent-decision-client-consumer-response.mjs`、`node --check scripts/validate-agent-decision-client-consumer-response.mjs`、`sh test/readme_ci_snippet_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/showcase_scripts_test.sh`、`go test ./...` 和 `git diff --check`。
6. [x] `f4b513d` 已提交并推送，main CI run `29934497118` passed，覆盖测试、server build、testgen build 和 Docker image build。
7. [x] `b1de301` 已提交并推送，main CI run `29934757726` passed，覆盖测试、server build、testgen build 和 Docker image build。
8. [x] `238b210` 已提交并推送，main CI run `29935050782` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零八阶段：导出包携带基础 Agent response 契约

1. [x] `scripts/export-agent-decision-fixtures.mjs` 已把基础客户端 CI summary/response schema、通过态/失败态 fixture、renderer 和无依赖 validator 一并复制到导出包。
2. [x] 导出包 `package.json` 已新增 `validate:client-summary`、`render:client-response` 和 `validate:client-response`，外部客户端不 checkout 整仓也能离线固定基础 `agent_next_step` 响应形状。
3. [x] `test/agent_decision_fixture_export_test.sh` 已断言新增文件存在，并在导出包临时目录内真实执行三条新增 npm script。
4. [x] README、客户端集成说明和 MCP 客户端契约测试说明已同步“导出包包含基础 response contract”的定位。
5. [x] 本地已通过 `sh test/agent_decision_fixture_export_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/mcp_client_contract_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/release_doc_index_test.sh`、`go test ./...`、`node --check scripts/export-agent-decision-fixtures.mjs` 和 `git diff --check`。
6. [x] `3bfb827` 已提交并推送，main CI run `29978751697` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百零九阶段：基础 Agent decision client 接入方样板

1. [x] 新增 `examples/agent-decision-client-adopter/`，提供外部客户端可复制的 README 和 `read-testloop-agent-decision-response.mjs` helper。
2. [x] 新增 `scripts/showcase-agent-decision-client-adopter.sh`，创建临时外部客户端仓库，导出 fixture 包，运行 `npm test --silent`，渲染并校验基础 response，再用接入方 helper 消费 `agent_next_step`。
3. [x] 新增 `test/agent_decision_client_adopter_example_test.sh`，覆盖 helper 通过态、失败态、缺文件分流和完整 showcase JSON 输出。
4. [x] `.github/workflows/ci.yml` 和 `test/showcase_scripts_test.sh` 已纳入新测试、bash 语法检查和 helper `node --check`。
5. [x] README、fixture 索引和客户端集成说明已同步基础接入方样板入口。
6. [x] 本地已通过 `sh test/agent_decision_client_adopter_example_test.sh`、`sh test/showcase_scripts_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/release_doc_index_test.sh`、`go test ./...`、`bash -n scripts/showcase-agent-decision-client-adopter.sh`、`node --check examples/agent-decision-client-adopter/scripts/read-testloop-agent-decision-response.mjs` 和 `git diff --check`。
7. [x] `932975f` 已提交并推送，main CI run `29979263727` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百一十阶段：基础接入样板 summary 契约化

1. [x] `scripts/showcase-agent-decision-client-adopter.sh --json` 已新增 `readme_path`，方便 Agent 从 summary 定位接入方样板文档。
2. [x] 新增 `docs/fixtures/agent-decision-client-adopter-summary.schema.json`、通过态 fixture 和 `invalid-response` 失败态 fixture。
3. [x] 新增 `scripts/validate-agent-decision-client-adopter-summary.mjs`，支持默认通过态校验、运行时 summary 校验和 `--json` 失败输出。
4. [x] 新增 summary schema/validator 测试，并把新 validator 接入 `.github/workflows/ci.yml` 与 `test/showcase_scripts_test.sh`。
5. [x] README、fixture 索引、客户端集成说明和基础接入样板 README 已同步 summary schema、fixture 和 validator 入口。
6. [x] 本地已通过 `sh test/agent_decision_client_adopter_summary_schema_test.sh`、`sh test/agent_decision_client_adopter_summary_validator_test.sh`、`sh test/agent_decision_client_adopter_example_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/readme_ci_snippet_test.sh`、`sh test/release_doc_index_test.sh`、`sh test/showcase_scripts_test.sh`、`sh test/ci_workflow_test.sh`、`sh test/docs_links_test.sh`、`go test ./...`、`node --check scripts/validate-agent-decision-client-adopter-summary.mjs`、`node --check examples/agent-decision-client-adopter/scripts/read-testloop-agent-decision-response.mjs`、`bash -n scripts/showcase-agent-decision-client-adopter.sh` 和 `git diff --check`。
7. [x] `0ff6392` 已提交并推送，main CI run `29979720950` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百一十一阶段：coverage task 比较符输入与分支断言

1. [x] `coverageTaskInputValues` 已支持从 SuggestedInputs/MissingBranches 解析 `>`、`>=`、`<`、`<=`、`!=`、`!==` 和 `is not` 条件。
2. [x] 覆盖任务输入归一化已能把开区间条件转成可执行示例值，例如 `count > 0` 生成 `1`、`offset < 10` 生成 `9`。
3. [x] `!=`/`is not` 条件已补齐 bool、整数、字符串和空值的反向示例值，JS/Python/Java/Rust 的 null-ish 值按语言生成非空占位。
4. [x] JS/Python 函数体边界识别已复用同一套条件归一化逻辑，coverage task 生成测试时能把比较符分支返回值映射到正确断言。
5. [x] 新增 helper 级和 JS/Python coverage task 生成级回归测试，覆盖 `count > 0` 生成 `classify(1)` 并断言 positive 分支。
6. [x] 本地已通过 `go test ./internal/generator`、`go test ./tools -run 'TestHandleGenerateTests|TestHandleValidateCoverageTask'`、`go test ./...` 和 `git diff --check`。
7. [x] `45871e8` 已提交并推送，main CI run `29980436262` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百一十二阶段：coverage task 复合 AND 条件输入与断言

1. [x] coverage task hint 解析已支持拆分 `&&` 和 Python `and` 连接的复合条件。
2. [x] JS/Python 的 if 边界识别已从单比较式提升为条件片段解析，`count > 0 && total < 10` / `count > 0 and total < 10` 会生成两个参数约束。
3. [x] JS/Python 分支返回值映射已同步支持复合条件，生成测试能用复合条件输入断言命中的分支返回值。
4. [x] 新增 helper 级、JS coverage task 生成级、Python coverage task 生成级回归测试，固定 `classify(1, 9)` 并断言 inside 分支。
5. [x] 本地已通过 `go test ./internal/generator`、`go test ./tools -run 'TestHandleGenerateTests|TestHandleValidateCoverageTask'`、`go test ./...` 和 `git diff --check`。
6. [x] `cbc2681` 已提交并推送，main CI run `29980897486` passed，覆盖测试、server build、testgen build 和 Docker image build。

当前目标：继续推进功能侧下一个高收益缺口；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。

## 第五百一十三阶段：coverage task 布尔条件输入

1. [x] coverage task hint 解析已支持 bare boolean、JS `!enabled` 和 Python `not enabled` 条件。
2. [x] JS/Python if 边界识别复用布尔条件解析，布尔取反分支能映射到正确返回值。
3. [x] JS 生成器保留可选构造类型参数的 `undefined` 行为，避免 `!transaction` 这类可选对象分支误生成 `false`。
4. [x] 新增 helper 级、JS coverage task 生成级、Python coverage task 生成级回归测试，固定 `classify(false)` / `classify(False)` 并断言 disabled 分支。
5. [x] 本地已通过 `go test ./internal/generator`、`go test ./tools -run 'TestHandleGenerateTests|TestHandleValidateCoverageTask'`、`go test ./...` 和 `git diff --check`。
