...
7. [ ] 下一步继续处理剩余 21 个普通失败：`TopicRouteData` 需要带可读/可写权限的非空 message queue，`PublishingSettings` / `ProducerOptions` 需要跨 constructor 的最小有效业务状态，`SettingsPB` 是 protobuf 导出名与源码 import alias 的映射问题，`SessionCredentials` 需要必填字段 mock。
