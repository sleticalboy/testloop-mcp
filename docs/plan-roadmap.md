...
已完成补充：新增 `Post-Release Verify` 手动 workflow。发布后输入 tag 后，会先校验 release 资产清单，再对 Linux amd64、Linux arm64、macOS arm64、Windows amd64 和 Windows arm64 执行安装脚本 dry run；Linux/macOS 会进一步运行已安装二进制的 `--help`，Windows 在 Linux runner 上完成 zip 下载、`.sha256` 校验和 `.exe` 安装检查。
