...
1. [ ] 从 RocketMQ 剩余失败中优先选择 `Endpoints`、`TopicRouteData`、`MessageOptions`、`ProducerOptions` 四类入参，按“读取同项目类型定义 -> 生成最小合法构造 -> 真实 Mocha 编译执行”闭环推进。
2. [ ] 优先实现通用能力：当参数类型是同项目导出的 class/interface/type alias 时，解析 import 来源并读取目标文件，复用已有 TS object mock 生成能力，而不是把未知类型统一退化成 `{}` 或 `undefined`。
3. [ ] 对 class 类型优先尝试可访问 constructor；对 interface/type alias 生成结构化对象；对 protobuf wrapper / gRPC client / 外部服务强依赖对象保留 `manual_review_internal` 或明确 public-entry 策略。
4. [ ] 验收顺序：先跑 RocketMQ 单文件过滤 `RpcClient.ts`、`PublishingMessage.ts`、`PublishingLoadBalancer.ts`、`Producer.ts`，再复跑 top100，目标是把 `repair_generated_test=33` 明显下降，同时避免引入项目专用硬编码。
