...
已完成补充：接入样板 summary 现在同时有 passed/failed 两类 fixture；失败态固定 `agent_next_step=inspect-release-smoke-summary`、`should_accept=false` 和 `failures[]`，validator `--json` 会把这些失败原因带给 Agent。下一步应跑相关测试、提交并观察 main CI。
