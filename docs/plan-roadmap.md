...
已完成补充：JS/TS 生成链路现在从公开入口、内部 helper 到用户文档都统一成“JavaScript/TypeScript coverage task”语义。默认无 task 生成仍保持 Jest 风格，带 task 时会根据 `framework` 选择 Jest/Vitest matcher 或 Mocha/Chai 断言，不改变既有输出契约。
