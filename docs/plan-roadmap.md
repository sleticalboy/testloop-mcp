...
1. [ ] 在 `examples/release-response-adopter/README.md` 增加 CI artifact 清单，覆盖 summary、response、consumer JSON 和 helper 输出。
2. [ ] 扩展 `scripts/showcase-release-response-adopter.sh --json` 输出或 README，明确 artifact 路径如何上传。
3. [ ] 增加测试固定 artifact 清单字段名和路径，避免接入方缺失关键证据。
4. [ ] README、client integration 或 checklist 同步 artifact 包说明。
5. [ ] 提交并观察 main CI。
