...
已完成补充：安装脚本在 GitHub 443 网络波动或 release asset 下载失败时，现在会明确提示下载失败和 GitHub 网络可达性检查，再回退到 `go install`；不支持的平台和缺少解压器也会给出对应原因。下一步可以继续触发 `Post-Release Verify` workflow 对 v0.4.11 做远端五平台安装 dry run，或回到 JS/TS 静态生成边界增强。
