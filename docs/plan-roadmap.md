...
已完成补充：Python regression smoke 不再依赖旧 `/tmp` Click task JSONL，且 Click ready 样本已从漂移的 parser 私有 helper 行段迁到当前 Click `8.2.1` 可稳定生成并运行的 utils 任务。下一步提交本轮改动并等待远端 CI。
