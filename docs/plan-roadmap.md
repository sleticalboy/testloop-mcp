...
已完成补充：安装与分发工作已收敛到 `v0.4.4`。当前 release matrix 覆盖 Linux amd64、Linux arm64、macOS arm64 和 Windows amd64，安装脚本支持 release 资产下载、checksum 校验、Windows zip 和 `go install` 回退，`sleticalboy/tap` 已升级到 `0.4.4` 并通过 Homebrew 验证。逐版本发布证据和后续维护流程统一记录在 `docs/plan-installation.md`。

## 第十五阶段：生态集成和编辑器体验

1. [ ] 评估是否需要 VS Code Extension 配套；当前 MCP 服务器已能通过通用 MCP 客户端接入，扩展应聚焦更低摩擦的本地配置、命令发现和结果展示，而不是重复 MCP 协议能力。
