...
已完成补充：第六真实样本证明 TypeScript/Jest 链路还需要覆盖 workspace pnpm 依赖布局、`ts-jest` diagnostics、ESM 动态导入、Jest mock hoisting、未导出顶层 helper 的公共入口覆盖、生成测试中的复杂 mock 类型约束、Node `process.platform/process.arch` 运行时全局状态分支，以及 TS 严格模式下的函数入参类型合法性。当前 `src/exec.ts` top30 已达到 `passed=30`，其中 28 个为真实 ready 测试，2 个为明确的 `manual_review_internal`。下一步建议继续扩大到 Codex SDK TS/Jest top40，重点验证 `prependPathDirs`、`pathEnvKey`、`isFile/isDirectory`、更多 native package path resolution 分支和 Windows PATH 兼容场景。
