...
6. [ ] 更新 `sleticalboy/homebrew-tap` 到 `testloop-mcp 0.5.8` 并推送。
7. [ ] 运行本机 Homebrew tap 验证。
8. [ ] 手动触发 Post-Release Verify，确认发布后五平台安装脚本 dry run 通过。

已完成补充：v0.5.8 的 GitHub Release 侧发布核验已完成，仓库 Formula 已更新到真实 digest。下一步更新 Homebrew tap 并跑本机 tap 验证；完成后触发 Post-Release Verify 做发布后闭环。
