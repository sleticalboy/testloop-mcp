...
已完成补充：first-run artifact 现在不仅能被文档解释，也能被可运行 demo 转成 Agent 应回复的四段结构。下一步建议补一个真实失败 artifact 的 fixture 包或小型 e2e，把 `run-first-run-ci.sh` 产出的五件套直接喂给 demo，验证从 CI 失败到 Agent 回复的整条链路。
