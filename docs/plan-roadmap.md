...
已完成补充：第六真实样本证明 TypeScript/Jest 链路还需要覆盖 workspace pnpm 依赖布局、`ts-jest` diagnostics、ESM 动态导入、Jest mock hoisting、未导出顶层 helper 的公共入口覆盖、生成测试中的复杂 mock 类型约束，以及 Node `process.platform/process.arch` 这类运行时全局状态分支。当前 `src/exec.ts` top20 已达到 `passed=20`，其中 18 个为真实 ready 测试，2 个为明确的 `manual_review_internal`：`findCodexPath` 的内部 platform package map 不一致分支和 optional native package 布局分支。下一步建议继续扩大到 Codex SDK TS/Jest top30，重点验证 `resolveNativePackage`、`existingDirs`、`pathEnvKey`、`isFile/isDirectory` 和更多 Node fs/path 相关分支。
