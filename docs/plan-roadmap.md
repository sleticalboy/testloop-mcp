...
已完成补充：Java ready 的定义从“生成测试可运行”推进到“生成测试可运行且目标 line_range 被 JaCoCo 确认覆盖”。下一步应优先回归剩余历史弱 ready 样本，例如 `CharSequenceUtils.toCharArray`、`Metaphone.metaphone`，确认命中校验能系统性拦截测试通过但未命中缺口的场景。
