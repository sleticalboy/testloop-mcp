...
已完成补充：第十四个真实样本把 Java/JUnit 链路从最小 smoke 推进到真实 Maven 多模块项目。它暴露并修复了 JaCoCo package path、嵌套 Maven module、深层测试路径、aggregator 执行、JUnit 4 风格、Checkstyle 约束、构造函数重载、enum 参数和集合异常分支这些真实 Java 项目中的高频问题。当前窗口仍只有 `route/Endpoints.java` top1，下一步应优先扩大到同一项目 top5/top10，观察普通方法、Optional/集合返回、异常路径、外部服务依赖和已有测试类合并策略；若 top window 中出现外部服务或路由状态依赖，应按项目定位归类为 `manual_review_external_service`，不要让静态生成器继续硬猜。
