...
已完成补充：Java ready 的定义从“生成测试可运行”推进到“生成测试可运行且目标 line_range 被 JaCoCo 确认覆盖”。当前已覆盖真实命中样本 `Failable`、`ClassUtils`、`CharSequenceUtils`，也覆盖了需要降级为手审的 `Metaphone`。下一步应给 Java 验证脚本补一个按 task id 精确筛选的能力，降低这类历史弱 ready 回归的执行成本。
