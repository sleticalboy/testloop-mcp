...
9. [x] 扩大到 RocketMQ Node.js TS/Mocha top100：脚本通过，`status_counts={"passed":100}`，`action_counts={"ready":67,"manual_review_private":33}`，`zero_skip=100`，`skipped_total=0`；普通 `repair_generated_test` 清零，任务覆盖 `Producer.ts`、`StatusChecker.ts`、`RpcClient.ts`、`BaseClient.ts`、`util/index.ts` 等真实业务文件。

已完成补充：第七真实样本证明 JS/TS 闭环不能假设 Mocha 项目都能用 `npx mocha` 直接运行。真实 TypeScript/Mocha 项目经常通过 `egg-bin`、`ts-node` 或项目脚本注册运行时，coverage runner 也可能是 c8/egg-bin 包装命令。当前验证脚本和 `run_tests` 已支持命令模板，使 Agent 能在不污染原项目的前提下复用项目自己的测试入口。RocketMQ Node.js top100 已达到 `passed=100`，其中 67 个为真实 ready，33 个为明确的 `manual_review_private`；这些 private 分支大多需要真实 RocketMQ/gRPC 服务或复杂内部状态，不应为了清零手审而生成不可维护的伪测试。下一步建议继续在 RocketMQ 样本上挑选一个不依赖外部服务的普通失败高风险区域，例如 `StatusChecker`、`MessageId`、`util` 或 retry/route 纯逻辑文件，扩大到文件级过滤样本，验证 TypeScript/Mocha 生成断言质量，而不是只看 topN 汇总通过。
