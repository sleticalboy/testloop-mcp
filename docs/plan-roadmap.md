...
已完成补充：Java ready 的定义从“生成测试可运行”推进到“生成测试可运行且目标 line_range 被 JaCoCo 确认覆盖”。当前已覆盖真实命中样本 `Failable`、`ClassUtils`、`CharSequenceUtils`，也覆盖了需要降级为手审的 `Metaphone`。Java 真实项目回归现在可以按 task id 精确筛选，下一步应把这个能力用于更细粒度的历史弱 ready 样本复查，或继续收敛 Java 验证脚本的耗时。
