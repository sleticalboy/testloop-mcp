...
16. [x] 新增 mcp-hub 真实 Vitest repair 样本：`ConfigManager.loadConfig` 空 config paths 分支固定回归 `failed/repair_generated_test`，并通过 `TESTLOOP_VALIDATE_JS_ALLOWED_FAILURE_ACTIONS` 只对显式期望的失败 action 放行，默认 top-N 验证仍保持严格。

已完成补充：固定 smoke 矩阵已从单语言 Java 扩展到 Java + JS + Python。当前默认矩阵重点保证真实项目小样本可以低成本复用历史任务并跑完整 `generate_tests -> run_tests -> parse/fix/coverage` 闭环。JS 侧这次避开了旧 ufo JSONL 的源码漂移，改用当前本机可运行的 ip2region JavaScript binding；同时确认 Jest 项目需要用 `--runTestsByPath` 避免生成测试名与既有测试文件模糊匹配。当前 JS smoke 已补上仓库内 TypeScript no-runtime、未导出 helper fixture，以及 mcp-hub 真实 Vitest repair 样本，稳定覆盖 `ready`、`manual_review_no_runtime`、`manual_review_internal`、`repair_generated_test` 四类输出；Python smoke 也补上 name-mangled private method internal fixture，不再只覆盖 ready。fixture task JSONL 已抽到统一 helper，manual-review runner 也已改成读取生成测试名并输出更接近真实框架的 skipped 摘要；后续新增仓库 fixture 或真实项目样本应优先扩展 preset 和 runner 合约，而不是在 regression 脚本里继续内联 JSON 或固定输出文案。后续可继续准备依赖完整的真实 Python 项目非 ready 样本，或者把 mcp-hub repair 失败进一步推进为可自动生成的异常断言。
