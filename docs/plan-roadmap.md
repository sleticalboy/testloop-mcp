...
1. [ ] 选择第九个真实样本，优先从应用型 TypeScript/Jest/Vitest 项目或 Python/pytest 项目中选一个，不再只选纯工具库，避免样本过度偏向无副作用函数。
2. [ ] 先跑基线测试和覆盖率命令，确认项目自己的 runner 可控；如果需要自定义命令，优先复用 `TESTLOOP_VALIDATE_JS_COVERAGE_COMMAND` / `TESTLOOP_JS_TEST_COMMAND`，不要为了验证污染原仓库。
3. [ ] 跑 top20 或当前任务上限，记录 `ready/manual_review/repair_generated_test/generation_error` 分布；任何普通 repair 都要先判断是不是通用生成缺口。
4. [ ] 对新增缺口补 parser/generator/runner 单元测试，再复跑真实样本，目标仍是普通 `repair_generated_test=0` 或明确归入 manual-review/external/no-runtime。
5. [ ] 复查前八个样本的指标表，整理一份简短中文质量报告，说明哪些能力已经稳定、哪些场景仍需要手审或真实服务环境。
