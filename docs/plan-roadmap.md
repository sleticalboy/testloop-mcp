...
1. [x] 新增 `scripts/showcase-onboarding-ci-external-project.sh`，在 `/tmp` 创建最小 Go 项目，并从该项目目录运行 onboarding CI bootstrap。
...
已完成补充：外部项目 Onboarding CI 演练已经通过真实临时 Go 项目验证，并修复了内部 MCP process smoke 对调用 cwd 的隐含依赖。下一步建议把这类“从外部用户目录运行”的约束沉到回归测试里：在 `run_onboarding_ci_test` 或新增轻量测试中模拟外部 cwd，防止后续 helper 脚本重新引入仓库 cwd 假设。
