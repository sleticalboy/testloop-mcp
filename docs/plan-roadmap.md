...
已完成补充：Release Artifacts workflow 已改为 matrix build，后续 tag release 会准备 Linux amd64、Linux arm64 和 macOS arm64 三类 tarball，并在单独发布 job 中统一生成 `checksums.txt`。Windows 预构建二进制因 CGO/tree-sitter 工具链验证成本继续暂缓，保留 `go install` 和源码构建路径。新增 `scripts/install.sh` 支持一键安装：优先下载匹配 release 资产并校验 checksum，资产缺失时自动回退到 `go install`。Homebrew tap 暂缓到版本节奏和资产命名更稳定后处理。
