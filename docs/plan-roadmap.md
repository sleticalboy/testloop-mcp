...
已完成补充：独立客户端 smoke 的脚本证据已经沉淀成可复制文档，接入方可以按目录结构把 release summary -> response -> Agent next step 固定到自己的 CI。下一步应跑文档/CI/Go 门禁、提交并等待 main CI；CI 通过后继续评估是否需要把这条 release response smoke 加入 README 的展示命令区或 release checklist。
