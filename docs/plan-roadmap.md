...
4. [x] 新增 `test/agent_decision_fixture_export_test.sh`，验证导出包内容、复制后的 JSON 校验、非空目录拒绝覆盖，并纳入 CI。
5. [x] README、客户端集成说明和 MCP 客户端契约测试说明已暴露 `node scripts/export-agent-decision-fixtures.mjs /tmp/testloop-agent-decision-fixtures`。
