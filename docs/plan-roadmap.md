...
1. [ ] 新增或整理 helper 输出字段说明，覆盖 `testloop_release_response_*` 和 `testloop_release_response_summary_*`。
2. [ ] 在接入样板 README 增加一张字段表，说明 status、next_step、should_accept、failures 的消费动作。
3. [ ] 增加文档测试固定两组 helper 输出字段，避免后续改名。
4. [ ] README、client integration 或 fixtures 索引补充 helper 输出入口。
5. [ ] 提交并观察 main CI。
