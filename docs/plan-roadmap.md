...
1. [ ] 选择第二个 Python 样本，优先选择非纯工具库、带 fixtures / monkeypatch / HTTP client / 数据库或项目自定义 pytest runner 的应用型项目。
2. [ ] 建立可重复 baseline：确认依赖安装方式、pytest 命令、coverage JSON 生成路径和需要的环境变量；如果全量测试依赖外部服务，先选稳定子集。
3. [ ] 使用 `scripts/validate-py-coverage-top-tasks.sh` 跑 top20 或当前任务上限，记录 `ready/manual_review/repair_generated_test/generation_error` 分布。
4. [ ] 对普通 `repair_generated_test` 先判断是否为通用 Python 生成缺口，再补 parser/generator/validator 回归测试；对真实外部依赖则归入明确的 `manual_review_*`。
5. [ ] 复跑样本并更新 `docs/real-project-validation.md`，目标是普通 repair 清零或全部可解释归类。
