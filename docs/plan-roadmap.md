...
已完成补充：第六真实样本证明 TypeScript/Jest 链路还需要覆盖 workspace pnpm 依赖布局、`ts-jest` diagnostics、ESM 动态导入、Jest mock hoisting、未导出顶层 helper 的公共入口覆盖、生成测试中的复杂 mock 类型约束、Node `process.platform/process.arch` 运行时全局状态分支、TS 严格模式下的函数入参类型合法性、间接触发内部 helper 时必须选择能真正走到目标分支的配置形状、无法从测试模块稳定触达的内部文件系统 helper 必须明确标记为手审，以及“能跑通”不等于“有价值覆盖”，需要为参数分支生成可观察的命令行/env/错误路径断言。当前 `src/exec.ts` top60 已达到 `passed=60`，其中 52 个为真实 ready 测试，8 个为明确的 `manual_review_internal`。下一步建议继续扩大到 Codex SDK TS/Jest top70，重点验证 `threadId`、`images`、`apiKey`、`isFile`、更多 native package path resolution 分支和 Windows PATH 兼容场景。
