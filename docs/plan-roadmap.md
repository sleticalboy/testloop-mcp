...
8. [ ] Post-Release Verify run `29157901152` 已触发但仍 queued，首个 `ubuntu-latest` job 尚未拿到 runner。

已完成补充：v0.4.14 已完成正式 GitHub Release 和 Homebrew 发布核验。当前剩余项不是代码或资产问题，而是等待 GitHub Actions runner 分配后完成 `Post-Release Verify` 五平台安装脚本 dry run。下一步建议继续跟进 run `29157901152`；如果长时间不启动，可取消后重触发一次，或者把 workflow 拆成资产校验与平台安装两个手动入口以降低排队影响。
