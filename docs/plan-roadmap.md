...
已完成补充：第十六个真实样本证明 Java 生成器不能只考虑“推荐测试文件路径”，还必须保护成熟项目已有的测试文件和 helper。Commons Codec 初跑暴露的关键问题不是断言质量，而是生成测试覆盖 `Base64Test.java` 后破坏上游测试类契约；现在生成器会选择 `Base64TestLoopTest.java` 这类不碰撞文件，并让 coverage task、上下文和类名保持一致。该样本也补上了嵌套类目标的通用能力：`Base64.Builder.setDecodeTableFormat` 能生成 `Base64.Builder` receiver、限定返回类型和按 line range 选择的 `Base64.DecodeTableFormat` 输入。当前 `Base64.java` top5 已达到 `passed=5/ready=2/manual_review_internal=3`，普通 repair 清零。下一步建议继续在 Commons Codec 扩大验证窗口，优先跑 `DigestUtils.java` top10 或 `Rule.java` top10，观察 CLI/static utility、hash/I/O helper、规则 parser 内部方法和外部资源依赖分类是否还能保持普通 repair 清零。
