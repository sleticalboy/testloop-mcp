...
1. [ ] 新增脚本或扩展 showcase，生成 `testloop-release-response-adopter-artifacts/` 目录。
2. [ ] artifact 目录包含 adopter summary、install summary、release response JSON、consumer JSON 和 summary consumer JSON。
3. [ ] 增加测试固定 artifact 目录文件清单。
4. [ ] 接入样板 README 和 client integration 同步打包 helper 命令。
5. [ ] 提交并观察 main CI。
