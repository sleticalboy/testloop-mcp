...
11. [x] 为新增能力补回归测试：覆盖 root package import、ASGI route scope、auth scope、临时配置文件读取和 bool/int cast，防止服务型 Python 项目重新退化为 `None` 或 basename import。
12. [x] 更新 `docs/real-project-validation.md` 和质量评估，把 Starlette 作为第十一个真实项目样本纳入当前质量边界。

已完成补充：第十一个真实样本把 Python/pytest 链路推进到 ASGI/HTTP 服务型场景。Starlette 样本暴露的通用问题包括仓库根包 import、coverage 配置污染测试文件、ASGI scope 最小输入、认证 scope 对象、配置文件读取和类型转换。当前 Starlette top20 已达到 `ready=20`、`skipped_total=0`。下一步建议继续扩大 Starlette 到 top30/top50，或切换到带数据库/外部服务依赖的 FastAPI 项目，重点验证依赖注入、monkeypatch 和 `manual_review_external_service` 分类。
