...
已完成补充：第十二个真实样本把 Python/pytest 链路推进到带业务依赖的 FastAPI 后端。它暴露了无 pyproject/test config 的项目根推断问题，以及文件/zip 输入、原地写入 dict helper、外部 SDK fake、对象存储依赖隔离、服务 helper 不能用 `None` 参数硬调、SQLAlchemy query 链最小 fake、HTMLResponse 分支断言、模块级 monkeypatch、API Key request fake、QR import fallback、数据库迁移 fake Session/inspect，以及动态定义路由不能伪装普通 ready 这些真实服务中常见的生成质量缺口。当前 haoy-apk-station backend top80 已达到 `ready=79`、`manual_review_environment=1`、`skipped_total=1`，普通生成失败清零。下一步建议继续扩大该样本到 top100，重点观察对象存储失败回滚、真实 APK 上传解析、JWT 错误路径和更深的数据库异常是否能稳定归类。
