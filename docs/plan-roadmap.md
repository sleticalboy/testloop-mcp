...
已完成补充：Java 真实样本已经开始从单文件收敛进入多文件扩展。新增 list-only 模式让后续选择验证窗口更稳，不必为了看 task 分布先跑完整 per-task 验证。`StatusChecker.java` top2 暴露的是另一类高频 Java 问题：静态方法、checked exception、protobuf builder、gRPC metadata 和 generic future/context 构造。当前这类问题已能转成可执行 JUnit 断言。下一步建议继续按 unfiltered top80 选择更小的文件窗口，例如 `hook/AttributeKey.java` top2 或 `impl/ClientType.java` top4；如果进入 `ClientImpl` / `ClientSessionImpl` 这类重依赖类，应优先识别外部服务、RPC session 或路由状态依赖，并稳定归类为手审，而不是继续堆静态构造特化。
