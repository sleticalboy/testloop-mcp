...
已完成补充：第六真实样本证明 TypeScript/Jest 链路还需要覆盖 workspace pnpm 依赖布局、`ts-jest` diagnostics、ESM 动态导入、Jest mock hoisting、未导出顶层 helper 的公共入口覆盖，以及生成测试中的复杂 mock 类型约束。当前 `src/exec.ts` top10 已达到 `ready=10`、`skipped_total=0`。下一步建议继续扩大到 Codex SDK TS/Jest top20，重点验证 `toTomlValue` 的数组、对象、null/unsupported 类型、TOML key formatting、native package path resolution 和 Node fs/path 相关分支。
