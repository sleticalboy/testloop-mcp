...
已完成补充：固定 smoke 矩阵已从单语言 Java 扩展到 Java + JS + Python。当前默认矩阵重点保证真实项目小样本可以低成本复用历史任务并跑完整 `generate_tests -> run_tests -> parse/fix/coverage` 闭环。JS 侧这次避开了旧 ufo JSONL 的源码漂移，改用当前本机可运行的 ip2region JavaScript binding；同时确认 Jest 项目需要用 `--runTestsByPath` 避免生成测试名与既有测试文件模糊匹配。当前 JS 非 ready 候选还不适合进入默认 smoke：ip2region 没有稳定手审样本，ufo 源码漂移，Codex SDK TypeScript 缺 Jest 依赖。下一步应优先补一个可随仓库运行的 JS no-runtime/internal fixture，或重新准备一个依赖完整的 JS/TS 真实项目样本。
