...
6. [x] 本地已通过 `sh test/agent_decision_fixtures_result_schema_test.sh`、`sh test/agent_decision_fixture_validator_test.sh`、`sh test/client_integration_doc_test.sh`、`sh test/docs_links_test.sh`、`go test ./...`、`sh test/release_doc_index_test.sh`、`node --check scripts/validate-agent-decision-fixtures.mjs` 和 `git diff --check`。
7. [ ] 提交、推送和 main CI 待执行。

当前目标：完成 validator JSON 契约化验证并提交推送；发布 tag、GitHub Release 和 Homebrew tap 后续单独处理。
