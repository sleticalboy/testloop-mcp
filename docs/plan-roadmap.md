...
5. [x] 新增 `scripts/validate-regression-smoke.sh`，默认串联 Java + Python 固定小回归，输出到统一 smoke 目录；Java 覆盖 ready / unreachable / internal 三类样本，Python 覆盖 pytest ready 样本。
...
7. [ ] 基于当前源码快照重新导出 JS/TS 样本 JSONL，再接入 `validate-regression-smoke.sh` 默认矩阵；旧 ufo JSONL 指向的 `src/*.ts` 在当前本地项目中已不存在，不适合作为固定样本。

已完成补充：固定 smoke 矩阵开始从单语言 Java 走向跨语言。当前默认矩阵先覆盖 Java + Python，重点保证真实项目小样本可以低成本复用历史任务并跑完整 `generate_tests -> run_tests -> parse/fix/coverage` 闭环。JS/TS 的底层复用能力已经补齐，但默认样本需要重新从当前源码快照导出，避免用旧临时 JSONL 引入源码漂移噪音。下一步应选择一个本机依赖完整、源码稳定的 JS/TS 项目，导出 2-3 条 ready / no-runtime / internal 样本后接入总入口。
