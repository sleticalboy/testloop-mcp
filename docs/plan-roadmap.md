...
已完成补充：Java 真实样本已经开始从单文件收敛进入多文件扩展。新增 list-only 模式让后续选择验证窗口更稳，不必为了看 task 分布先跑完整 per-task 验证。`StatusChecker.java` top2 暴露的是静态方法、checked exception、protobuf builder、gRPC metadata 和 generic future/context 构造；`AttributeKey.java` top2 暴露的是私有构造器和静态工厂选择；`ClientType.java` top4 暴露的是 enum body 递归解析和 enum 常量 receiver 生成；`InflightRequestCountInterceptor.java` top2 暴露的是接口参数最小实现、hook enum 输入和 void 方法状态副作用断言。当前这些问题都已转成可执行 JUnit 断言。下一步建议继续按 unfiltered top80 进入 `CompositedMessageInterceptor.java` 这种组合 hook 文件；如果进入 `ClientImpl` / `ClientSessionImpl` 这类重依赖类，应优先识别外部服务、RPC session 或路由状态依赖，并稳定归类为手审，而不是继续堆静态构造特化。
