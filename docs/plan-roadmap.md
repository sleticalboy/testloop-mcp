...
1. [ ] 选定一个最小外部客户端样板目录，不污染主仓库发布资产。
2. [ ] 用 `v0.5.20` release response installer 生成可提交的 workflow、客户端包和安装 summary。
3. [ ] 固定接入方 README，说明如何运行 `npm test --silent`、如何读取 `testloop-release-response.json` 和如何按 `agent_next_step` 分流。
4. [ ] 增加或复用回归测试，确保样板命令不会漂移。
5. [ ] 把样板接入证据记录到 `docs/real-integration-cases.md` 或专门的 `docs/plan-*` 文档。
