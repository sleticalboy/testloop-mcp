...
已完成补充：公开 onboarding demo 现在不只是在终端串跑三个命令，还能留下 Markdown、summary JSON 和 Agent decision 三类制品；失败样例也已固定为可执行 fixture。下一步建议评估是否把 onboarding report wrapper 放进 GitHub Actions 示例，减少接入方手写 env 和 artifact 路径。
