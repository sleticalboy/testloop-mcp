...
已完成补充：`v0.4.1` 已发布到 GitHub Release，tag 指向 `c69e717bf9da5c783bad5d1928d29d97b89deb79`；Release Artifacts run `28739889556` 已上传 Linux amd64 压缩包和 `checksums.txt`；`go install @latest` 已验证可安装主服务和 `testgen` CLI。

已完成补充：Release Artifacts workflow 已改为 matrix build，后续 tag release 会准备 Linux amd64、Linux arm64、macOS arm64 和 Windows amd64 产物。新增 `scripts/install.sh` 支持一键安装：优先下载匹配 release 资产并校验 checksum，资产缺失时自动回退到 `go install`。Homebrew tap 已接入 `sleticalboy/tap`。

已完成补充：`v0.4.2` 已发布到 GitHub Release，包含 Linux amd64、Linux arm64、macOS arm64 三类 tarball 和 `checksums.txt`。安装脚本已验证可从 `v0.4.2` Release 直接下载 macOS arm64 资产并安装主服务和 `testgen` CLI。

已完成补充：`v0.4.3` 已发布到 GitHub Release，包含 Linux amd64、Linux arm64、macOS arm64 三类 tarball 及 Windows amd64 zip，并为每个单资产提供 `.sha256`。

已完成补充：`v0.4.4` 已发布到 GitHub Release，正式 release matrix 覆盖 Linux amd64、Linux arm64、macOS arm64 和 Windows amd64，安装脚本已支持 Windows zip 资产，`sleticalboy/tap` 已升级到 `0.4.4` 并通过 `brew test`。
