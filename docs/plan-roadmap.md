...
已完成补充：安装与分发工作已收敛到 `v0.4.9`。当前 release matrix 覆盖 Linux amd64、Linux arm64、macOS arm64、Windows amd64 和 Windows arm64，安装脚本支持 release 资产下载、checksum 校验、Windows zip 和 `go install` 回退，`sleticalboy/tap` 已升级到 `0.4.9` 并通过 Homebrew 验证。逐版本发布证据和后续维护流程统一记录在 `docs/plan-installation.md`。
...
1. [ ] 为 `fix_suggestions` 增加更多真实框架失败 fixture，覆盖 Go、pytest、Jest/Vitest、Mocha 的断言失败、panic/exception 和超时场景。
2. [ ] 增加面向 Agent 的 repair task 输出，把失败分类、源码上下文、建议命令和可编辑文件聚合为稳定 JSON 契约。
3. [ ] 让 `run_tests` 在失败结果中可选附带 `fix_suggestions` 摘要，减少 Agent 额外往返。
4. [ ] 为 repair task 增加 golden tests，固定字段、排序和降级行为。
