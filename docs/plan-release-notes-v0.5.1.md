...
- [ ] 正式版本准备时更新 `main.go` MCP implementation version 到 `0.5.1`。
- [ ] 正式版本准备时将 `CHANGELOG.md` 的 `Unreleased` 内容收敛为 `v0.5.1 - 2026-07-17`。
- [ ] 正式版本准备时更新 README、安装文档和必要的版本引用。
- [ ] 正式发布前重新跑远端 CI、Release Artifacts、资产校验和 Homebrew 安装链路验证。
...
- 这不是正式发布记录；当前没有切 `main.go` implementation version，也没有更新安装文档中的已发布版本引用。
