...
- [ ] 候选边界整理提交后的远端 CI passed。
- [ ] 正式版本准备文件尚未更新：implementation version、`CHANGELOG.md` 正式版本段和安装/接入文档版本引用仍停留在 `0.5.15` / `v0.5.15`。
- [ ] 尚未打 `v0.5.16` tag，尚未创建 GitHub Release、Release assets 或 Homebrew tap 更新。

## 摘要

v0.5.16 继续沿着“面向 AI 编程代理的测试反馈闭环 MCP 服务”推进。这个候选版本不扩语言，也不承诺测试生成算法提升；重点是让外部 MCP 客户端、编辑器插件和 AI Coding Agent 集成方，可以把 v0.5.15 新增的 Agent 决策 fixture 包真正放进自己的 CI。

v0.5.15 解决的是“fixture 包能导出、能校验”。v0.5.16 候选解决的是“客户端仓库怎么复制、怎么 dry-run、怎么让 CI / Agent 直接读 JSON 结果”。
...
- 当前模板仍使用 `ref: main` checkout helper，因为该模板是 v0.5.15 之后新增；正式发布 v0.5.16 后应建议接入方固定到 `v0.5.16` tag。
...
- 正式发布后，应把模板里的 helper `ref: main` 调整或说明为推荐固定到 `v0.5.16`。
