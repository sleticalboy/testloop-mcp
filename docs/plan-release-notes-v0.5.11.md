...
- [x] 本地 release readiness 预检已通过，覆盖完整测试矩阵、候选二进制构建、help/version、darwin arm64 打包 dry-run、sha256 和 tarball 内容检查。
- [x] 正式版本准备已更新 `main.go` MCP implementation version 到 `0.5.11`。
- [x] 正式版本准备已将 `CHANGELOG.md` 的 `Unreleased` 内容收敛为 `v0.5.11 - 2026-07-19`。
- [x] 正式版本准备已同步 README、安装文档和必要版本引用到 `v0.5.11`。
- [x] 正式版本准备本地完整验证已通过。
- [x] 正式版本准备提交 `473e764` 远端 CI run `29675557908` passed。
- [x] tag `v0.5.11` 已推送。
- [x] Release Artifacts tag-push run `29675619230` 因 runner 长时间 queued 已取消；手动 dispatch run `29676083347` passed。
- [x] `scripts/verify-release-assets.sh v0.5.11` 已验证 10 个 Release 资产完整。
- [x] GitHub Release `v0.5.11` 正文已更新为正式发布说明，并标记为 latest。
- [x] 仓库内 Formula 已生成到 `0.5.11`，`ruby -c Formula/testloop-mcp.rb` 通过。
- [x] Formula/发布记录提交 `aea0330` 远端 CI run `29677030693` passed。
- [x] Homebrew tap 已升级到 `0.5.11`，tap commit `e810f60` 已推送。
- [x] 本机 `brew audit --formula --strict sleticalboy/tap/testloop-mcp` 通过；本机直连 release asset 下载曾因 GitHub 443/传输中断失败，最终以远端 Post-Release Verify 作为安装门禁。
...
- [x] `/tmp/testloop-mcp-v0.5.11-candidate --version`
- [x] `/tmp/testloop-mcp-v0.5.11-candidate --help`
- [x] `/tmp/testloop-testgen-v0.5.11-candidate --help`
- [x] 正式版本准备后 `/tmp/testloop-mcp-v0.5.11-release-prep --version` 输出 `testloop-mcp 0.5.11`
...
- [x] `shasum -a 256 -c testloop-mcp_v0.5.11_darwin_arm64.tar.gz.sha256`
- [x] `tar -tzf /tmp/testloop-v0.5.11-candidate-dist/testloop-mcp_v0.5.11_darwin_arm64.tar.gz`
...
- [x] `scripts/verify-release-assets.sh v0.5.11`
- [x] `ruby -c Formula/testloop-mcp.rb`
- [x] `brew audit --formula --strict sleticalboy/tap/testloop-mcp`
- [x] Post-Release Verify run `29679705790`

## 发布前待办

- [x] 完成候选发布检查清单 `docs/plan-release-v0.5.11.md`。
- [x] 正式版本准备时更新 `main.go` MCP implementation version 到 `0.5.11`。
- [x] 将 `CHANGELOG.md` 的 `Unreleased` 内容收敛为 `v0.5.11 - 2026-07-19`。
- [x] 同步 README、installation、quickstart 和接入指南中的版本引用到 `0.5.11` / `v0.5.11`。
- [x] 跑完整发布前门禁和 release readiness。
- [x] 提交版本准备后等待远端 CI。
- [x] 打 `v0.5.11` tag，生成 Release 资产，更新 GitHub Release。
- [x] 生成仓库内 Formula。
- [x] 更新 Homebrew tap。
- [x] 手动触发 Post-Release Verify，确认五平台安装脚本 dry run 通过。
