...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | expanded2 top231 | `passed=231`，`ready=228`，`manual_review_environment=3` | 0 |
...
haoy-apk-station 的 pytest baseline 已扩到 `7 passed`，关键文件覆盖率提升到 `apps.py=67.4%`、`auth.py=73.3%`、`auth_service.py=81.8%`；当前剩余 coverage task 池缩小到 231 个并已全部验证通过。下一步更有价值的是继续补该样本的剩余业务边界：发布说明长度限制、设置当前版本、应用详情/版本列表 not found、下载流式成功路径、登录禁用/密码错误、删除用户 self/not found；如果这些边界的新增缺口变少，则切换第十三个真实样本。
