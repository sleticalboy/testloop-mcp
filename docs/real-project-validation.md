...
Python/pytest 链路已经从基础 pytest 草稿推进到真实 `src` layout 项目：能处理 coverage report 相对路径映射、dotted package import、单行安全 task 注释、class `__init__` 元数据、constructor 入参、常见 fallback/error path 输入、wrapper swallow exception、Unicode fallback、stream fallback 和内部状态对象构造。
...
下一步更有价值的样本不是继续选择纯 utility 库，而是第二个 Python 应用型项目，重点覆盖 pytest fixtures、monkeypatch、HTTP client、数据库策略和项目自定义测试入口。
