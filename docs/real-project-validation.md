...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top120 | `passed=120`，`ready=118`，`manual_review_environment=2` | 0 |
...
下一步更有价值的是继续扩大业务型 Python 服务样本到 top140，重点观察真实 APK 上传解析后半段、对象存储失败回滚、应用详情/版本详情的更深数据库查询、JWT 错误路径和统计聚合是否能被稳定归类为 ready 或 `manual_review_external_service` / `manual_review_database`。
