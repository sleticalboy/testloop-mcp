...
`Digest.java` top10 全部集中在 CLI `Digest.run` 的多个行段：`93`、`105-114`、`143` 等。复跑达到 `passed=10/manual_review_internal=10`，普通 repair 清零。这一轮没有新增 ready 能力，但确认 CLI run 的内部循环、输出和文件/目录处理路径不会被静态生成器伪造成脆弱单测，而是稳定给出可运行手审 skip。下一步应继续扩大 Commons Codec 到 `Blake3.java` 或更大的跨文件窗口，观察二进制状态对象、Path/File/InputStream 和外部资源输入是否还能保持普通 repair 清零。
