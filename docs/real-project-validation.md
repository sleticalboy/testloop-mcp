...
| 14 | RocketMQ Java client | Java / Maven + JUnit + JaCoCo | `route/Endpoints.java` top1 | `passed=1`，`ready=1`，`skipped_total=1` | 0 |
...
Rust 已补齐 opt-in top-task 验证入口，并用最小 Cargo crate + 自定义 LCOV writer 跑通 `passed=2/ready=2` 的 smoke；但由于本机 `cargo-tarpaulin` 不存在、`llvm-tools-preview` 下载未完成，该结果不计入真实项目样本表。Java 已从最小 Maven/JUnit smoke 推进到真实 Maven 多模块项目验证：RocketMQ Java client 的 `route/Endpoints.java` top1 使用真实 JaCoCo XML、真实 Maven reactor 和真实 JUnit runner 跑通 `passed=1/ready=1`。该样本暴露并修复了 JaCoCo package path 到嵌套 Maven module 的映射、深层 Java 包路径找不到 `pom.xml`、Maven module 需要从 aggregator 根目录使用 `-pl/-am` 执行、JUnit 4/5 风格识别、license header / Checkstyle、构造函数重载按行号过滤，以及 `AddressScheme.DOMAIN_NAME.equals(scheme)` 这类 enum/list/异常分支输入构造问题。下一步应扩大 Java 真实验证窗口，优先从同一项目 top1 扩到 top5/top10，观察非构造函数、集合/Optional、异常路径和外部服务依赖的 action 分布。
