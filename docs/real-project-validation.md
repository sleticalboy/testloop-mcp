...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top80 | `passed=80`，`ready=79`，`manual_review_environment=1` | 0 |
...
下一步更有价值的是继续扩大业务型 Python 服务样本到 top100，重点观察用户管理/API Key 的剩余权限分支、对象存储失败回滚、真实 APK 上传解析、JWT 错误路径和更深的数据库异常是否能被稳定归类为 ready 或 `manual_review_external_service` / `manual_review_database`。
