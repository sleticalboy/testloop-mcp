...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top20 | `passed=20`，`ready=20`，`skipped_total=0` | 0 |
...
下一步更有价值的是扩大业务型 Python 服务样本到 top40，重点观察数据库异常、对象存储失败、下载重定向、短链访问、隐藏应用、统计聚合和外部服务依赖是否能被稳定归类为 ready 或 `manual_review_external_service` / `manual_review_database`。
