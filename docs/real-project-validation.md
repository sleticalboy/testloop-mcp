...
`HmacUtils.java` 同样只有 7 个候选任务。初跑为 `passed=1/failed=6`，其中 `isAvailable("test")` 断言方向错误，`new HmacUtils("test", "test")` 使用了不存在算法名，`instance.hmac(null)` / `hmacHex(null)` 在 `Path`、`String` 或 `ByteBuffer` 重载间产生编译歧义或运行时空实例问题。修复后生成器对 HmacUtils 使用 `HmacAlgorithms.HMAC_SHA_256`、稳定 key 和 typed `java.nio.ByteBuffer.wrap(...)` 输入，复跑达到 `passed=7/ready=7`，普通 repair 清零。下一步应继续扩大 Commons Codec 到更多 crypto/path/stream API，例如 `Digest.java`、`Blake3.java` 或更大的跨文件窗口，观察 I/O error path 和外部资源输入是否还能保持普通 repair 清零。
