...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top267 | `passed=267`，`ready=264`，`manual_review_environment=3` | 0 |
...
下一步更有价值的是先扩充 haoy-apk-station 的 pytest baseline，让覆盖率任务池继续产生真实 APK 上传成功/失败、上传后数据库一致性和统计聚合缺口；如果该样本短期无法继续扩展，则切换第十三个真实样本，继续验证业务型 Python/JS/Go 项目的闭环质量。
