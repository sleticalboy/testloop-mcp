...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | expanded3 top208 | `passed=208`，`ready=205`，`manual_review_environment=3` | 0 |
...
haoy-apk-station 的 pytest baseline 已扩到 `10 passed`，关键文件覆盖率提升到 `apps.py=79.9%`、`auth.py=82.5%`、`auth_service.py=83.0%`；当前剩余 coverage task 池缩小到 208 个并已全部验证通过。下一步更有价值的是继续补该样本的剩余上传、对象存储和 API Key 管理边界；如果新增缺口继续变少，则切换第十三个真实样本。
