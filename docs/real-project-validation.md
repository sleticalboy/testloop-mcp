...
Rust 已补齐 opt-in top-task 验证入口，并用最小 Cargo crate + 自定义 LCOV writer 跑通 `passed=2/ready=2` 的 smoke；但由于本机 `cargo-tarpaulin` 不存在、`llvm-tools-preview` 下载未完成，该结果不计入真实项目样本表。Java 已补齐 opt-in top-task 验证入口，并用最小 Maven/JUnit 项目 + 自定义 JaCoCo XML writer 跑通 `passed=1/ready=1` 的 smoke，验证了 JaCoCo 包路径映射到 `src/main/java`、任务隔离复制、JUnit 测试生成和 runner 解析。第十四个真实样本仍应优先选择真实 Rust crate 或真实 Java 项目，并使用真实 tarpaulin/llvm-cov LCOV 或 JaCoCo XML。
