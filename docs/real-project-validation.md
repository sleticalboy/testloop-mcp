...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top60 | `passed=60`，`ready=60`，`skipped_total=0` | 0 |
...
下一步更有价值的是继续扩大业务型 Python 服务样本到 top80，重点观察用户管理、API Key 管理、JWT 错误路径、对象存储失败回滚和更深的数据库异常是否能被稳定归类为 ready 或 `manual_review_external_service` / `manual_review_database`。
