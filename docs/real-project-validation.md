...
haoy-apk-station 的 pytest baseline 已扩到 `13 passed`，关键文件覆盖率提升到 `apps.py=80.6%`、`auth.py=88.3%`、`storage.py=82.1%`；当前剩余 coverage task 池缩小到 168 个并已全部验证通过。该样本核心业务闭环已经覆盖较充分，下一步更有价值的是切换第十三个真实样本，避免继续在低层对象存储和 APK parser 细节上收益递减。
