...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top140 | `passed=140`，`ready=138`，`manual_review_environment=2` | 0 |
...
下一步更有价值的是继续扩大业务型 Python 服务样本到 top160，重点观察上传后对象存储成功/失败回滚、更多版本删除和应用详情更新路径、JWT 错误路径，以及统计聚合的更深 SQLAlchemy 链是否能稳定归类为 ready 或 `manual_review_external_service` / `manual_review_database`。
