...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | expanded top248 | `passed=248`，`ready=245`，`manual_review_environment=3` | 0 |
...
下一步更有价值的是继续扩充 haoy-apk-station 的 pytest baseline，优先加入下载统计聚合、版本删除/应用删除后的数据库一致性、短链隐藏/无版本页面，以及认证刷新/禁用用户路径；如果该样本继续扩展的边际收益下降，则切换第十三个真实样本。
