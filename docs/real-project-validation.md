...
| 12 | haoy-apk-station backend | Python / FastAPI + pytest | top200 | `passed=200`，`ready=197`，`manual_review_environment=3` | 0 |
...
下一步更有价值的是继续扩大业务型 Python 服务样本到 top220，重点观察上传后成功/失败回滚、更多应用详情更新路径、删除/移动文件后的数据库一致性，以及统计聚合的更深 SQLAlchemy 链是否能稳定归类为 ready 或 `manual_review_external_service` / `manual_review_database`。
