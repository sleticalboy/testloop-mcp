...
如果要看外部仓库可直接照抄的基础消费样板，见 [Agent decision client 接入方样板](../examples/agent-decision-client-adopter/README.md)；维护者可运行 `scripts/showcase-agent-decision-client-adopter.sh --json` 验证导出包、response renderer、validator 和接入方 helper 的完整基础闭环。该 summary 可用 `node scripts/validate-agent-decision-client-adopter-summary.mjs /path/to/agent-decision-client-adopter-summary.json` 校验，结构契约见 [agent-decision-client-adopter-summary.schema.json](./fixtures/agent-decision-client-adopter-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-adopter-summary/passed.json)，失败态样例见 [invalid-response.json](./fixtures/agent-decision-client-adopter-summary/invalid-response.json)。
...
维护者可以运行 `scripts/showcase-agent-decision-client-ci-template-install.sh --json`，验证下载安装脚本、生成 workflow、执行 contract、渲染基础 response 并校验 response artifact 的完整外部客户端 dry-run；JSON 输出结构见 [Agent 决策客户端 CI 模板安装 summary schema](./fixtures/agent-decision-client-ci-template-install-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-ci-template-install-summary/passed.json)，其中 `response_json`、`response_validation_json` 和 `response_validator_exit_code` 固定基础 response 校验结果。客户端不想引入 JSON Schema 工具链时，可以运行 `node scripts/validate-agent-decision-client-ci-install-summary.mjs /path/to/install-summary.json` 做无依赖校验。
...
consumer response 的结构契约见 [Agent 决策客户端 consumer response schema](./fixtures/agent-decision-client-consumer-response.schema.json)，通过态和失败态 fixture 见 [passed.json](./fixtures/agent-decision-client-consumer-response/passed.json)、[client-summary-validator-failed.json](./fixtures/agent-decision-client-consumer-response/client-summary-validator-failed.json)、[validator-failed.json](./fixtures/agent-decision-client-consumer-response/validator-failed.json) 和 [fixture-drift.json](./fixtures/agent-decision-client-consumer-response/fixture-drift.json)；response evidence 会保留基础 response validator 的 `client_response_validator_json` 和 `client_response_validator_exit_code`，无依赖校验入口是 `node scripts/validate-agent-decision-client-consumer-response.mjs /path/to/consumer-response.json`。消费端 summary 失败态 fixture 见 [client-summary-validator-failed.json](./fixtures/agent-decision-client-consumer-smoke-summary/client-summary-validator-failed.json)、[validator-failed.json](./fixtures/agent-decision-client-consumer-smoke-summary/validator-failed.json) 和 [fixture-drift.json](./fixtures/agent-decision-client-consumer-smoke-summary/fixture-drift.json)，适合客户端固定失败分流测试。
