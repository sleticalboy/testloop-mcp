...
如果希望测试自动发现 artifact fixture，可以读取 [agent-response-artifact-manifest.json](./fixtures/agent-response-artifact-manifest.json)，其中固定了 first-run / onboarding 的目录、必备文件、期望 action、`expected_section_signals` 和 fallback 顺序。manifest 的 JSON Schema 见 [agent-response-artifact-manifest.schema.json](./fixtures/agent-response-artifact-manifest.schema.json)，适合客户端生成类型或做契约校验。summary JSON 的结构契约见 [verification-summary.schema.json](./fixtures/verification-summary.schema.json)，其中 `sections[].signals.action` 可用于读取 section 级动作信号。`verification-summary-decision-demo` 会先校验 `overall_status`、`failed_count` 和 `sections` 这些必填字段，再输出 `agent_next_step`，避免把非 verification summary 的 JSON 误判成 ready。当前 fixture 会要求客户端保留 `独立 CLI 生成动作 smoke:manual_review`，但该 signal 不等于整体验收失败。
...
该 demo 会同时验证 `agent-response.txt`、`agent-decision.txt`、`verification-summary.json`、summary schema 和 manifest 中的 `expected_section_signals`，并输出 `decision_action=...` 与 `summary_validated=verification-summary.json`，方便客户端确认机器分流、回复草稿和 summary 契约一致。
