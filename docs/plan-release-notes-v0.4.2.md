...
- [x] 远端 CI passed
- [x] Release Artifacts build jobs 已验证 Linux amd64、Linux arm64 和 macOS arm64 均可构建
- [x] `v0.4.2` Release 已包含三平台 tarball 和 `checksums.txt`
- [x] 安装脚本已验证可从 `v0.4.2` Release 直接下载 macOS arm64 资产

## 后续修正

- 发布后发现单独 publish job 可能长时间等不到 runner。后续 workflow 已改为每个 matrix build job 直接上传本平台 tarball 和 `.sha256`，安装脚本也兼容单资产 `.sha256`。
