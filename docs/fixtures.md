...
Release response 接入方样板 summary 的 JSON 结构见 [release-response-adopter-summary.schema.json](./fixtures/release-response-adopter-summary.schema.json)，通过态样例见 [passed.json](./fixtures/release-response-adopter-summary/passed.json)，对应 `scripts/showcase-release-response-adopter.sh --json` 输出；可运行 `node scripts/validate-release-response-adopter-summary.mjs` 做无依赖校验。通过态会固定 `status=passed`、`release_ref=v0.5.20`、`fixture_count=8`、`agent_next_step=ready`、`should_accept=true` 和 `npm_exit_code=0`。
