...
基础 Agent 决策客户端接入方样板见 [Agent decision client 接入方样板](../examples/agent-decision-client-adopter/README.md)，对应 `scripts/showcase-agent-decision-client-adopter.sh --json` 输出。它会创建临时外部客户端仓库，导出最小 fixture 包，运行 `npm test --silent`，渲染并校验基础 response，再用接入方 helper 读取 `testloop_agent_decision_response_next_step`，用于验证外部客户端不 checkout 整仓时也能消费基础 Agent response。
