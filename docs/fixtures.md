...
Agent 决策客户端 CI 模板安装 dry-run 的 JSON 摘要结构见 [agent-decision-client-ci-template-install-summary.schema.json](./fixtures/agent-decision-client-ci-template-install-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-ci-template-install-summary/passed.json)，对应 `scripts/showcase-agent-decision-client-ci-template-install.sh --json` 输出；它会同时返回 `summary_json`、`response_json`、`response_validation_json` 和 `response_validator_exit_code`，可运行 `node scripts/validate-agent-decision-client-ci-install-summary.mjs` 做无依赖校验。

基础 Agent 决策客户端 CI summary 的 JSON 摘要结构见 [agent-decision-client-ci-summary.schema.json](./fixtures/agent-decision-client-ci-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-ci-summary/passed.json)，对应 `scripts/showcase-agent-decision-client-ci.sh --json` 输出。它会指向导出包内的 `agent-decision-fixtures-result.json` 与 `agent-decision-fixtures-result.schema.json`，方便接入方同时固定基础 summary 和 validator result 两层契约；可运行 `node scripts/validate-agent-decision-client-ci-summary.mjs` 做无依赖校验。

基础 Agent 决策客户端 CI response 的 JSON 结构见 [agent-decision-client-ci-response.schema.json](./fixtures/agent-decision-client-ci-response.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-ci-response/passed.json)，失败态样例见 [validator-failed.json](./fixtures/agent-decision-client-ci-response/validator-failed.json) 和 [fixture-drift.json](./fixtures/agent-decision-client-ci-response/fixture-drift.json)，对应 `scripts/render-agent-decision-client-ci-response.mjs --json` 输出。它固定 `ready`、`inspect-client-validator` 和 `inspect-agent-decision-fixtures` 分流，适合客户端直接纳入 response 解析单元测试；可运行 `node scripts/validate-agent-decision-client-ci-response.mjs` 做无依赖校验。

基础 Agent 决策客户端接入方样板 summary 的 JSON 结构见 [agent-decision-client-adopter-summary.schema.json](./fixtures/agent-decision-client-adopter-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-adopter-summary/passed.json)，失败态样例见 [invalid-response.json](./fixtures/agent-decision-client-adopter-summary/invalid-response.json)，对应 `scripts/showcase-agent-decision-client-adopter.sh --json` 输出。它会创建临时外部客户端仓库，导出最小 fixture 包，运行 `npm test --silent`，渲染并校验基础 response，再用接入方 helper 读取 `testloop_agent_decision_response_next_step`，用于验证外部客户端不 checkout 整仓时也能消费基础 Agent response；可运行 `node scripts/validate-agent-decision-client-adopter-summary.mjs` 做无依赖校验。helper 输出字段表见 [Agent decision client 接入方样板](../examples/agent-decision-client-adopter/README.md)。

Agent 决策客户端消费端 smoke 的 JSON 摘要结构见 [agent-decision-client-consumer-smoke-summary.schema.json](./fixtures/agent-decision-client-consumer-smoke-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-consumer-smoke-summary/passed.json)，失败态样例见 [client-summary-validator-failed.json](./fixtures/agent-decision-client-consumer-smoke-summary/client-summary-validator-failed.json)、[validator-failed.json](./fixtures/agent-decision-client-consumer-smoke-summary/validator-failed.json) 和 [fixture-drift.json](./fixtures/agent-decision-client-consumer-smoke-summary/fixture-drift.json)，对应 `scripts/showcase-agent-decision-client-consumer-smoke.sh --json` 输出。它比安装 dry-run 多校验一层接入方消费路径：安装 summary、基础客户端 CI summary、基础客户端 CI response、导出 fixture manifest 和 `agent-decision-fixtures-result.json` 必须互相一致，并会生成 `client_response_validator_json` 和 `agent_response_json`；可运行 `node scripts/validate-agent-decision-client-consumer-smoke-summary.mjs` 做无依赖校验，也可运行 `node scripts/render-agent-decision-client-consumer-response.mjs` 把 summary 转成 `agent_next_step`。

Agent 决策客户端 consumer response 的 JSON 结构见 [agent-decision-client-consumer-response.schema.json](./fixtures/agent-decision-client-consumer-response.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-consumer-response/passed.json)，失败态样例见 [client-summary-validator-failed.json](./fixtures/agent-decision-client-consumer-response/client-summary-validator-failed.json)、[validator-failed.json](./fixtures/agent-decision-client-consumer-response/validator-failed.json) 和 [fixture-drift.json](./fixtures/agent-decision-client-consumer-response/fixture-drift.json)，对应 `scripts/render-agent-decision-client-consumer-response.mjs --json` 输出。它固定 `ready`、`inspect-consumer-smoke-validator` 和 `inspect-agent-decision-fixtures` 分流，evidence 会保留 `client_response_json`、`client_response_validator_json` 和 `client_response_validator_exit_code`，适合客户端直接纳入 response 解析单元测试；可运行 `node scripts/validate-agent-decision-client-consumer-response.mjs` 做无依赖校验。

Agent 决策客户端 release smoke 的 JSON 摘要结构见 [agent-decision-client-release-smoke-summary.schema.json](./fixtures/agent-decision-client-release-smoke-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-release-smoke-summary/passed.json)，对应 `scripts/showcase-agent-decision-client-release-smoke.sh --json` 输出。它用于正式发布后把 release tag raw installer、基础客户端 CI response 和 consumer smoke response 合成一份 Agent 可消费证据，正常结果应同时固定 `helper_refs.install=v0.5.21`、`helper_refs.consumer=v0.5.21`、`fixture_count=8` 和 `agent_next_steps.client/consumer=ready`。如果接入方需要最小消费器，可运行 `node scripts/render-agent-decision-client-release-response.mjs /path/to/release-smoke-summary.json`，把 summary 转成稳定的 `agent_next_step`。
...
Agent 决策 release response 客户端安装 summary 的 JSON 结构见 [agent-decision-release-response-client-install-summary.schema.json](./fixtures/agent-decision-release-response-client-install-summary.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-release-response-client-install-summary/passed.json)，对应 `scripts/install-agent-decision-release-response-client.sh --json` 输出；可运行 `node scripts/validate-agent-decision-release-response-client-install-summary.mjs` 做无依赖校验。通过态会固定 `status=written`、`release_ref=v0.5.21`、`fixture_count=8`、`agent_next_step=ready` 和 `npm_exit_code=0`。

Release response 接入方样板 summary 的 JSON 结构见 [release-response-adopter-summary.schema.json](./fixtures/release-response-adopter-summary.schema.json)，通过态样例见 [passed.json](./fixtures/release-response-adopter-summary/passed.json)，失败态样例见 [invalid-response.json](./fixtures/release-response-adopter-summary/invalid-response.json)，对应 `scripts/showcase-release-response-adopter.sh --json` 输出；可运行 `node scripts/validate-release-response-adopter-summary.mjs` 做无依赖校验。通过态会固定 `status=passed`、`release_ref=v0.5.21`、`fixture_count=8`、`agent_next_step=ready`、`should_accept=true` 和 `npm_exit_code=0`。失败态会固定 `status=failed`、`should_accept=false`、非 `ready` 的 `agent_next_step` 和 `failures[]`，接入方可用 `node scripts/validate-release-response-adopter-summary.mjs --json /path/to/summary.json` 读取规范化失败输出，也可复制 `examples/release-response-adopter/scripts/read-testloop-release-response-summary.mjs` 直接把 summary 转成 `testloop_release_response_summary_*` 键值给 Agent。下载 `testloop-release-response-adopter-artifacts/` 后可运行 `node scripts/verify-release-response-adopter-artifact.mjs /path/to/testloop-release-response-adopter-artifacts` 做离线自检；verifier JSON 契约见 [release-response-adopter-artifact-verification.schema.json](./fixtures/release-response-adopter-artifact-verification.schema.json)，通过态样例见 [passed.json](./fixtures/release-response-adopter-artifact-verification/passed.json)，失败态样例见 [missing-summary-consumer.json](./fixtures/release-response-adopter-artifact-verification/missing-summary-consumer.json)，可运行 `node scripts/validate-release-response-adopter-artifact-verification.mjs` 校验。最小客户端消费示例见 `go run ./examples/release-response-adopter-artifact-demo docs/fixtures/release-response-adopter-artifact-verification/passed.json`。helper 输出字段表见 [Release response 接入方样板](../examples/release-response-adopter/README.md)。
...
- `metadata.next_action_kind`
- `metadata.next_action_reason`
- `metadata.needs_better_input_reason`
- `metadata.manual_review_kind`
