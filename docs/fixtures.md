...
Agent 决策客户端 consumer response 的 JSON 结构见 [agent-decision-client-consumer-response.schema.json](./fixtures/agent-decision-client-consumer-response.schema.json)，通过态样例见 [passed.json](./fixtures/agent-decision-client-consumer-response/passed.json)，失败态样例见 [client-summary-validator-failed.json](./fixtures/agent-decision-client-consumer-response/client-summary-validator-failed.json)、[validator-failed.json](./fixtures/agent-decision-client-consumer-response/validator-failed.json) 和 [fixture-drift.json](./fixtures/agent-decision-client-consumer-response/fixture-drift.json)，对应 `scripts/render-agent-decision-client-consumer-response.mjs --json` 输出。它固定 `ready`、`inspect-consumer-smoke-validator` 和 `inspect-agent-decision-fixtures` 分流，适合客户端直接纳入 response 解析单元测试。
