...
如果 `testloop-mcp --version` 报 `flag provided but not defined: -version`，或版本不是当前文档中的 `0.5.5`，通常说明本机已经安装过旧二进制，或者 `PATH` 指到了旧位置。先刷新 tap 并升级：
...
```bash
TESTLOOP_MCP_VERSION=v0.5.5 sh scripts/install.sh
...
```bash
TESTLOOP_MCP_INSTALL_DIR="$USERPROFILE/bin" TESTLOOP_MCP_VERSION=v0.5.5 sh scripts/install.sh
```

维护者调试平台选择时也可以显式覆盖检测结果：

```bash
TESTLOOP_MCP_OS=windows TESTLOOP_MCP_ARCH=amd64 TESTLOOP_MCP_VERSION=v0.5.5 sh scripts/install.sh
TESTLOOP_MCP_OS=windows TESTLOOP_MCP_ARCH=arm64 TESTLOOP_MCP_VERSION=v0.5.5 sh scripts/install.sh
```
...
当前 `v0.5.5` Release 已提供以下产物：

- `testloop-mcp_v0.5.5_linux_amd64.tar.gz`
- `testloop-mcp_v0.5.5_linux_amd64.tar.gz.sha256`
- `testloop-mcp_v0.5.5_linux_arm64.tar.gz`
- `testloop-mcp_v0.5.5_linux_arm64.tar.gz.sha256`
- `testloop-mcp_v0.5.5_darwin_arm64.tar.gz`
- `testloop-mcp_v0.5.5_darwin_arm64.tar.gz.sha256`
- `testloop-mcp_v0.5.5_windows_amd64.zip`
- `testloop-mcp_v0.5.5_windows_amd64.zip.sha256`
- `testloop-mcp_v0.5.5_windows_arm64.zip`
- `testloop-mcp_v0.5.5_windows_arm64.zip.sha256`

```bash
curl -LO https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.5/testloop-mcp_v0.5.5_linux_amd64.tar.gz
curl -LO https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.5/testloop-mcp_v0.5.5_linux_amd64.tar.gz.sha256
sha256sum -c testloop-mcp_v0.5.5_linux_amd64.tar.gz.sha256
tar -xzf testloop-mcp_v0.5.5_linux_amd64.tar.gz
...
```powershell
$arch = "amd64"
curl.exe -LO "https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.5/testloop-mcp_v0.5.5_windows_$arch.zip"
curl.exe -LO "https://github.com/sleticalboy/testloop-mcp/releases/download/v0.5.5/testloop-mcp_v0.5.5_windows_$arch.zip.sha256"
$expected = (Get-Content ".\testloop-mcp_v0.5.5_windows_$arch.zip.sha256").Split()[0]
$actual = (Get-FileHash ".\testloop-mcp_v0.5.5_windows_$arch.zip" -Algorithm SHA256).Hash.ToLower()
if ($actual -ne $expected) { throw "checksum mismatch" }
Expand-Archive ".\testloop-mcp_v0.5.5_windows_$arch.zip"
& ".\testloop-mcp_v0.5.5_windows_$arch\testloop-mcp.exe" --help
& ".\testloop-mcp_v0.5.5_windows_$arch\testloop-testgen.exe" --help
```
...
```bash
scripts/generate-homebrew-formula.sh v0.5.5
...
```bash
scripts/update-homebrew-tap.sh v0.5.5 ../homebrew-tap
```

不传 `tap-dir` 时，脚本会把 `sleticalboy/homebrew-tap` 克隆到临时目录并更新公式。默认不会自动提交；确认无误后可用以下环境变量提交和推送 tap 仓库：

```bash
TESTLOOP_MCP_TAP_COMMIT=1 TESTLOOP_MCP_TAP_PUSH=1 scripts/update-homebrew-tap.sh v0.5.5 ../homebrew-tap
```
...
```bash
TESTLOOP_MCP_VERIFY_EXPECT_VERSION=0.5.5 scripts/verify-client-setup.sh /absolute/path/to/testloop-mcp
```
