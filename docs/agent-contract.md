...
当请求带 `coverage=true` 时，`validate_coverage_task` 会尽量校验 `coverage_task.line_range` 是否真正被生成测试命中。目前已覆盖 Jest/Vitest/Mocha 的 Istanbul `coverage/coverage-final.json`、`node-test` 的 TAP coverage raw output、pytest 项目根目录的 `coverage.json` 和 Java/JUnit 的 JaCoCo XML。若测试命令通过但目标行仍在未覆盖列表中，结果会降级为 `failed/needs_better_input`，并在 `metadata` 中返回 `coverage_target_hit=false`、`coverage_missed_lines` 和 `coverage_miss_reason`。
