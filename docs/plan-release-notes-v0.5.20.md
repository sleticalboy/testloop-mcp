...
- validator 固定 `status=written`、`release_ref=v0.5.19`、`fixture_count=8`、`agent_next_step=ready`、`npm_exit_code=0` 和决策序列。
...
- `go test ./...`
- `git diff --check`
- `TESTLOOP_RELEASE_CANDIDATE_DIST_DIR=/tmp/testloop-v0.5.19-goal-readiness-dist scripts/verify-release-candidate.sh v0.5.19`
...
- 发布前仍需正式版本准备：更新 implementation version、收敛 changelog、同步版本引用、复跑 release readiness、等待 main CI，再打 tag。
