...
覆盖率命中校验当前支持 Jest/Vitest/Mocha 的 Istanbul `coverage/coverage-final.json`、`node-test` 的 TAP coverage raw output、pytest 项目根目录的 `coverage.json`、Rust/Cargo 的 LCOV 和 Java/JUnit 的 JaCoCo XML。客户端看到 `metadata.coverage_target_hit=false` 时，应优先把它当作输入/入口不足，而不是测试运行器失败。
