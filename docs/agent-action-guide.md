...
覆盖率命中校验当前支持 Java/JUnit 的 JaCoCo XML、`node-test` 的 TAP coverage raw output 和 pytest 项目根目录的 `coverage.json`。客户端看到 `metadata.coverage_target_hit=false` 时，应优先把它当作输入/入口不足，而不是测试运行器失败。
