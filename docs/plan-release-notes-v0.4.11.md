...
- [x] GitHub Release 已发布：https://github.com/sleticalboy/testloop-mcp/releases/tag/v0.4.11
- [x] Release Artifacts workflow 已通过：`28995989142`
- [x] 五个平台资产和对应 `.sha256` 已上传并通过 `scripts/verify-release-assets.sh v0.4.11` 验证。
- [x] Homebrew tap 已更新到 `0.4.11`，并通过 `brew fetch`、`brew audit --strict`、`brew upgrade`、`brew test`。
...
- 这是 post-v0.4.10 的正式发布资料，不回写已经发布的 `docs/plan-release-notes-v0.4.10.md`。
- 本轮主要是静态生成质量和回归保护增强，不改变 MCP 工具协议。
- fake `npx` handler 闭环用于 CI 中的可运行性契约检查，不代表已经引入真实 npm/Vitest 安装依赖。
- 本机 GitHub 443 连接在安装脚本验证期间出现过超时和 HTTP/2 framing 错误；Release 资产直连下载、checksum、解包内容和 Homebrew 安装路径均已验证，`scripts/install.sh` 在网络不稳定时可回退 `go install` 并安装可运行命令。
