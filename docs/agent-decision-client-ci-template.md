...
当前模板 checkout `v0.5.20` tag 上的 helper，确保客户端 CI 使用稳定的 fixture 导出脚本、JSON 输出合同和外部仓库 dry-run 已验证过的相对路径。
...
脚本在仓库内运行时默认从 `main.go` 读取当前版本并生成 `ref: v0.5.20`；脱离仓库单文件运行时会回退到内置稳定 ref。如果需要固定到其他 tag 或预览写入路径：

```bash
scripts/install-agent-decision-client-ci-template.sh --version v0.5.20 /absolute/path/to/client-repo
