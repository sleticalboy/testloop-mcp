...
- [x] 版本准备提交远端 CI run `29629563807` passed。
- [x] Release Artifacts run `29629630932` passed。
- [x] `scripts/verify-release-assets.sh v0.5.2`
- [x] GitHub Release 正文已更新。
- [x] `scripts/generate-homebrew-formula.sh v0.5.2`
- [x] `ruby -c Formula/testloop-mcp.rb`
- [x] `brew style Formula/testloop-mcp.rb`
- [x] `TESTLOOP_MCP_TAP_COMMIT=1 TESTLOOP_MCP_TAP_PUSH=1 scripts/update-homebrew-tap.sh v0.5.2 /Users/binlee/code/open-source/homebrew-tap`
- [x] Homebrew tap 提交 `c1945e8 testloop-mcp 0.5.2` 已推送。
- [x] Post-Release Verify run `29629793877` passed，资产清单和五平台安装脚本 dry run 全部通过。
- [x] 本机 Homebrew tap 已快进到 `c1945e8`，`HOMEBREW_NO_AUTO_UPDATE=1 brew fetch --force sleticalboy/tap/testloop-mcp` 成功。

## 发布备注

- v0.5.2 适合作为“安装验收 + 真实 MCP 协议 smoke + 公开 showcase 决策断言”的版本节点。
- 发布文案应突出 Agent 接入可验证、公开演示可复验，以及 action 决策信号不会静默漂移。
- v0.5.2 已完成正式发布核验：远端 CI、Release Artifacts、GitHub Release 正文、资产校验、Homebrew tap、Post-Release Verify 和本机 `brew fetch` 均已通过。
