...
| 生成通用双项目报告 | `scripts/showcase-dual-project-report.sh "$(command -v testloop-mcp)"` | 否，脚本入口回归 | 提供任意两条 user-project smoke 的通用底座，输出两份报告和嵌套子 summary 的总状态文件；`TESTLOOP_PAIR_SUMMARY_JSON` 需要指向普通文件路径。 |
| 生成 laoxia 双栈验收报告 | `scripts/showcase-laoxia-scaffold-report.sh "$(command -v testloop-mcp)"` | 否，脚本入口回归 | 一次生成 `car-admin-server` 和 `car-admin-web` 两份验收报告，以及一份嵌套子 summary 的总 `laoxia-summary.json`；`TESTLOOP_LAOXIA_SUMMARY_JSON` 需要指向普通文件路径。 |
