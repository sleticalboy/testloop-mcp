...
| 生成 laoxia 双栈验收报告 | `scripts/showcase-laoxia-scaffold-report.sh "$(command -v testloop-mcp)"` | 否，脚本入口回归 | 一次生成 `car-admin-server` 和 `car-admin-web` 两份验收报告，适合作为真实项目双栈 smoke 的固定入口。 |
