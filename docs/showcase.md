...
| 演练外部项目 Onboarding CI | `scripts/showcase-onboarding-ci-external-project.sh` | 否，脚本入口回归 | 在 `/tmp` 创建非 testloop Go 项目，从该项目目录运行 bootstrap，验证复制路径不依赖仓库 cwd。 |
