...
| 首跑诊断 CI 模板 | `scripts/run-first-run-ci.sh 'go test ./...'` | 否，脚本入口和文档回归 | 面向外部用户项目 CI，自动准备 helper checkout 并上传 report、summary、summary schema、decision、context、Agent response 和 log。 |
...
| 校验 Agent artifact 目录 | `sh scripts/verify-agent-artifact.sh first-run /tmp/testloop-first-run` | 是，脚本回归 | 对下载后的 first-run/onboarding artifact 目录做离线校验，确认必备文件、summary schema、decision、Agent response 和 section signal 一致。 |
| 演练外部项目首跑诊断 CI | `scripts/showcase-first-run-ci-external-project.sh` | 否，脚本入口回归 | 在 `/tmp` 创建非 testloop Go 或 Node 项目，从该项目目录运行 first-run bootstrap，验证复制路径能产出七件套 artifact，并先拦截坏的输出目录输入。 |
...
| 演练外部项目 Onboarding CI | `scripts/showcase-onboarding-ci-external-project.sh` | 否，脚本入口回归 | 在 `/tmp` 创建非 testloop Go 或 Node 项目，从该项目目录运行 bootstrap，验证复制路径能产出五件套 artifact，并先拦截坏的输出目录输入。 |
...
| 生成 APK Info Rust/Words Java 双项目报告 | `TESTLOOP_PAIR_PREFIX=rustjava ... scripts/showcase-dual-project-report.sh /tmp/testloop-mcp` | 否，脚本入口回归 | 用 Rust workspace 与 Java Maven 项目验证 shared helper 的跨语言复用性，输出 `rustjava-summary.json` 及两份子 summary。 |
...
Onboarding showcase 证明首次接入路径可以从安装验收走到 Agent 闭环。`doctor-first-run.sh` 是更适合安装后直接运行的入口：它复用 onboarding report，但会稳定输出 `first_run_status`、`first_run_agent_next_step`、可粘贴上下文和 artifact 路径。`run-first-run-ci.sh` 是外部用户项目 CI 的 bootstrap 入口，会准备 testloop-mcp helper checkout 并上传首跑诊断七件套；`verify-agent-artifact.sh` 则用于下载 artifact 后做离线目录校验，确认 summary schema、decision、Agent response 和 section signal 一致。`showcase-first-run-ci-external-project.sh` 则从非 testloop 项目目录验证这条复制路径确实能产出 report、summary、summary schema、decision、context、Agent response 和 log，默认跑 Go，也可以用 `TESTLOOP_EXTERNAL_FIRST_RUN_PROJECT_TYPE=node|all` 验证 web/Node 命令形态。`showcase-agent-onboarding-report.sh` 适合公开录屏、接入方验收和 CI artifact：它复用验收报告脚本生成 Markdown 与 summary JSON，再运行 summary 决策 demo，把 `agent_next_step` 单独落盘；`render-onboarding-agent-response.sh` 可继续把 onboarding artifact 目录转成 Agent 四段回复。`showcase-onboarding-ci-external-project.sh` 证明 onboarding bootstrap 在非 testloop 项目目录中也能生成 report、summary、summary schema、decision 和 Agent response 五件套，适合发布后或改 onboarding 模板时手动复验；默认跑 Go，也可以用 `TESTLOOP_EXTERNAL_ONBOARDING_PROJECT_TYPE=node|all` 验证 web/Node 命令形态。`showcase-dual-project-report.sh` 提供任意两条 user-project smoke 的通用底座；`showcase-laoxia-scaffold-report.sh` 只是带默认值的 thin wrapper。`QuickSmoke` Go/Java 和 `APK Info` Rust/Java 复验说明这条 shared helper 不只适用于同类项目，也能把跨语言 pair 收成统一 summary。Go showcase 证明 `validate_coverage_task` 可以在外部 Go 项目上给出 `passed/ready` 决策信号，并默认校验该信号不漂移。JS/TS showcase 证明 Agent 不应只看测试是否通过，还要读取 `action`：`ready` 可以进入下一个任务，`manual_review_internal` 应记录手审或寻找公共入口；脚本也会默认校验这两个 action。公开项目 showcase 都支持通过 `TESTLOOP_SHOWCASE_*_PROJECT_DIR` 复用本地 checkout，减少外网 clone 对演示的影响；远端 clone/fetch 默认 60 秒超时，可通过 `TESTLOOP_SHOWCASE_*_GIT_TIMEOUT` 调整。
