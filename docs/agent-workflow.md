...
- `passed` / `manual_review_private`：生成结果命中 JavaScript `#private` method 的语言访问限制，外部测试不能直接调用；生成器会输出 skipped 复核草稿，metadata 可能包含 `public_entry_candidates`，应改为公共入口测试、重构可见性或人工复核。
- `passed` / `manual_review_internal`：生成结果命中 JavaScript ESM 内部未导出的 class/function；外部测试不能直接导入构造，应通过已导出的公共 API、测试 seam 或模块级集成测试覆盖。
- `failed` / `manual_review_private`：旧式或外部 provider 生成结果仍直接调用 JavaScript `#private` method；不要继续修补同一个直接调用测试，应改为公共入口测试或人工复核。
