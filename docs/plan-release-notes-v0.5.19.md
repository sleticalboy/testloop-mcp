...
- [x] `v0.5.19` tag 已推送，Release Artifacts run `29827625494` passed，五个平台 10 个资产已上传。
- [x] `TESTLOOP_MCP_REPO=sleticalboy/testloop-mcp scripts/verify-release-assets.sh v0.5.19` 已验证正式 Release 资产完整。
- [x] GitHub Release 正文已更新为正式 v0.5.19 发布说明。
- [x] 仓库内 Homebrew Formula 已用正式 Release asset digest 更新到 `0.5.19`。
- [x] Homebrew tap 已更新到 `0.5.19` 并推送，tap commit `72123db`。
- [x] Post-Release Verify run `29828306451` passed，覆盖资产 manifest、Linux amd64/arm64、macOS arm64 和 Windows amd64/arm64 安装校验。
- [x] 发布后 raw installer smoke、基础客户端 CI response smoke 和 consumer smoke 已通过，均固定 `helper_ref=v0.5.19` 或 `agent_next_step=ready`。
...
- v0.5.19 已完成 tag、Release assets、资产校验、GitHub Release 正文、仓库内 Formula、Homebrew tap、Post-Release Verify 和发布后 smoke。
