...
Java 真实项目链路已经用 RocketMQ Java client 跑通一个小窗口：`route/Endpoints.java` top1 在真实 Maven reactor、真实 JaCoCo XML 和真实 JUnit runner 下达到 `passed=1/ready=1`。这一轮把 Java 生成器从骨架进一步推进到项目规范可运行：能复用源文件 license header 和 package，按 Maven 依赖识别 JUnit 4/5，避免 star import，生成 `public class/public void` 以兼容旧 Surefire，按 coverage line range 选择重载构造函数，并能把 `AddressScheme.DOMAIN_NAME.equals(scheme)` 解析为 enum 输入，结合 `List<Address>` 生成异常分支断言。当前 Java 样本窗口仍很小，不能外推为通用 Java 项目质量；下一步应扩大到 top5/top10，并观察 Optional、集合、非静态方法、异常路径和外部服务依赖的普通 repair。
