...
Java 真实项目链路已经用 RocketMQ Java client 跑通一个小窗口：`route/Endpoints.java` top20 在真实 Maven reactor、真实 JaCoCo XML 和真实 JUnit runner 下达到 `passed=20/ready=20`，第二个文件 `exception/StatusChecker.java` top2 达到 `passed=2/ready=2`、`zero_skip=2`，普通 repair 清零。`Endpoints` 的 `skipped_total=20` 来自该项目现有测试套件每次固定 1 个 upstream skipped test，不是生成测试跳过；`StatusChecker` 没有 skipped。这一轮把 Java 生成器从骨架进一步推进到项目规范可运行：能复用源文件 license header 和 package，按 Maven 依赖识别 JUnit 4/5，避免 star import 并按项目 Checkstyle 要求排序 import，生成 `public class/public void` 以兼容旧 Surefire，按 coverage line range 选择重载构造函数，coverage task 下保留 getter/equals/hashCode 目标，为无默认构造器的实例方法选择可用构造器，并能把 `AddressScheme.DOMAIN_NAME.equals(scheme)`、`addresses.isEmpty`、`addresses.size`、空值路径、protobuf `Endpoints` builder 输入、`toSocketAddresses` 集合/空值返回、`equals` 细分返回路径、protobuf `Status` 和 `RpcFuture` 上下文构造转成可执行断言。当前 Java 样本窗口仍只覆盖两个文件，不能外推为通用 Java 项目质量；下一步应扩大到更多 Java 文件，观察 `toProtobuf`、Optional/外部 protobuf 类型和外部服务依赖的普通 repair 或手审分类。
