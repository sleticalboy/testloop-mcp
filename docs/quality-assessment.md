...
Java 真实项目链路已经用 RocketMQ Java client 从单文件扩展到多文件小窗口：`route/Endpoints.java` top20 在真实 Maven reactor、真实 JaCoCo XML 和真实 JUnit runner 下达到 `passed=20/ready=20`，`exception/StatusChecker.java` top2、`hook/AttributeKey.java` top2、`impl/ClientType.java` top4、`hook/InflightRequestCountInterceptor.java` top2、`hook/CompositedMessageInterceptor.java` top2、consumer `Assignment.java` top2、`Assignments.java` top2、`ConsumeTask.java` top1 和 `ConsumeService.java` top1 均达到 `ready`；`ConsumerImpl.java` top20 达到 `passed=20`、`ready=9`、`manual_review_internal=11`，其中 `wrapFilterExpression` 已通过 `wrapReceiveMessageRequest` 公共路径生成 SQL/TAG filter 断言，两个 `wrapReceiveMessageRequest` overload 已生成 auto-renew / invisible-duration request builder 断言，`changeInvisibleDuration` 已通过 `PushConsumerImpl` spy、mock `ClientManager`、OK/非 OK/失败 `RpcFuture` 覆盖 success/error/failure callback，`ackMessage` 已通过 mock `ClientManager.ackMessage` 覆盖返回路径，剩余 `wrapAckMessageRequest`、`wrapChangeInvisibleDuration` 和 `receiveMessage` 相关任务稳定归为 `manual_review_internal`；`impl/ClientManagerImpl.java` top5、`impl/ClientSessionImpl.java` top7 和 `impl/ClientImpl.java` top8 也稳定归为 `manual_review_internal`，普通 repair 清零。hook/route/enum/consumer 文件的 `skipped_total` 来自该项目现有测试套件每次固定 1 个 upstream skipped test 或手审测试；`ConsumerImpl` 的 ready 生成测试自身均为 `Tests run: 1, Skipped: 0`。这一轮把 Java 生成器从骨架进一步推进到项目规范可运行：能复用源文件 license header 和 package，按 Maven 依赖识别 JUnit 4/5，避免 star import 并按项目 Checkstyle 要求排序 import，生成 `public class/public void` 以兼容旧 Surefire，按 coverage line range 选择重载构造函数，coverage task 下保留 getter/equals/hashCode 目标，为无默认构造器的实例方法选择可用构造器，并能在私有构造器场景下选择 public static factory；当实例方法所在类只有复杂显式构造器且没有安全 factory 时，会生成可运行的 `manual_review_internal`，避免伪造 `new ClassName()`；未知自定义引用类型参数默认使用 `null`，避免伪造跨包类型无参构造和缺失 import；source import 现在只按测试代码实际引用复制，coverage 注释里的类型名不会触发无用 import。Java parser 现在也能递归进入 enum body declaration，生成器能把 enum 常量作为 receiver 覆盖 `toProtobuf` 这类分支；对 hook 状态型 void 方法，也能使用 `MessageInterceptorContextImpl` 最小实现和 `MessageHookPoints.RECEIVE` 输入生成状态副作用断言，并清理未使用 assertion import；对组合 hook，也能生成匿名 `MessageInterceptor`、非空 interceptor 列表和 `doAfter` 所需的 attribute map 前置状态；对 RocketMQ consumer 行为类，可以复用 `TestBase.fakeMessageViewImpl()`、lambda listener、Mockito interceptor、匿名抽象子类和 executor/scheduler fixture 覆盖 `ConsumeTask.call`、`ConsumeService.consume`、`ConsumerImpl.wrapFilterExpression`、`ConsumerImpl.wrapReceiveMessageRequest`、`ConsumerImpl.changeInvisibleDuration` 和 `ConsumerImpl.ackMessage` 的可见调用路径；对 Java private/internal 方法、抽象 client lifecycle、RPC session、request observer 和 stream lifecycle 分支，会生成可运行的手审 skip，避免伪造直接调用或输出空测试类；Java 手审 metadata 也已改为 Java 语境，不再复用 JavaScript “not exported” 文案。其他已验证能力包括 `AddressScheme.DOMAIN_NAME.equals(scheme)`、`addresses.isEmpty`、`addresses.size`、空值路径、protobuf `Endpoints` builder 输入、`toSocketAddresses` 集合/空值返回、`equals` 细分返回路径、protobuf `Status` 和 `RpcFuture` 上下文构造。当前 Java 样本窗口仍集中在 RocketMQ client，不能外推为通用 Java 项目质量；下一步应聚焦 `receiveMessage` response switch 和外部 protobuf response，判断能否复用现有 `okReceiveMessageResponsesFuture` 生成真实 ready，还是继续保留为 stream/RPC lifecycle 手审分类。
