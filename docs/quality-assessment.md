...
Java 真实项目链路已经用 RocketMQ Java client 跑通一个小窗口：`route/Endpoints.java` top10 在真实 Maven reactor、真实 JaCoCo XML 和真实 JUnit runner 下达到 `passed=10/ready=10`，普通 repair 清零。`skipped_total=10` 来自该项目现有测试套件每次固定 1 个 upstream skipped test，不是生成测试跳过。这一轮把 Java 生成器从骨架进一步推进到项目规范可运行：能复用源文件 license header 和 package，按 Maven 依赖识别 JUnit 4/5，避免 star import，生成 `public class/public void` 以兼容旧 Surefire，按 coverage line range 选择重载构造函数，coverage task 下保留 getter/equals/hashCode 目标，为无默认构造器的实例方法选择可用构造器，并能把 `AddressScheme.DOMAIN_NAME.equals(scheme)`、`addresses.isEmpty`、`addresses.size`、空值路径和 protobuf `Endpoints` builder 输入转成可执行断言。当前 Java 样本窗口仍只覆盖 `Endpoints.java`，不能外推为通用 Java 项目质量；下一步应扩大到 top20，并观察 `toSocketAddresses`、`toProtobuf`、集合返回、Optional/外部 protobuf 类型和外部服务依赖的普通 repair。
