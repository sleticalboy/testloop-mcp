...
Python/pytest coverage task 当前已验证过两个真实 `src` layout 项目，能处理 pytest-cov `coverage.json`、包路径 import、class constructor 元数据、常见 fallback/error path 输入、环境依赖手审分类、内部状态机、Pydantic 通知对象、dataclass 输入类型、keyword-only 参数和同步/异步 stream finish 状态。它仍需要更多服务型样本验证 fixtures、monkeypatch、数据库、HTTP client 和项目自定义 runner。
