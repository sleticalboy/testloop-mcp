...
Python/pytest coverage task 当前已验证过真实 `src` layout 项目，能处理 pytest-cov `coverage.json`、包路径 import、class constructor 元数据、常见 fallback/error path 输入和环境依赖手审分类。它仍需要更多应用型样本验证 fixtures、monkeypatch、数据库、HTTP client 和项目自定义 runner。
