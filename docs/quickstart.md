...
如果 `--version` 不存在或版本不是 `0.5.17`，先升级 Homebrew 安装：
...
```bash
TESTLOOP_FIRST_RUN_EXPECT_VERSION=0.5.17 \
  scripts/doctor-first-run.sh "$(command -v testloop-mcp)"
```
...
```bash
TESTLOOP_MCP_VERIFY_EXPECT_VERSION=0.5.17 scripts/verify-client-setup.sh "$(command -v testloop-mcp)"
```
