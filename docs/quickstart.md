...
如果 `--version` 不存在或版本不是 `0.5.13`，先升级 Homebrew 安装：
...
```bash
TESTLOOP_FIRST_RUN_EXPECT_VERSION=0.5.13 \
  scripts/doctor-first-run.sh "$(command -v testloop-mcp)"
```
...
```bash
TESTLOOP_MCP_VERIFY_EXPECT_VERSION=0.5.13 scripts/verify-client-setup.sh "$(command -v testloop-mcp)"
```
...
这个 demo 会展示 `run_tests.action=apply_fix_suggestions -> fix_suggestions.category -> repair_task -> rerun.action=ready -> parse_coverage` 的最小 Agent 消费路径。
