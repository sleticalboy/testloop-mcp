...
- [ ] 正式版本准备时更新 `main.go` MCP implementation version 到 `0.5.5`。
- [ ] 正式版本准备时将 `CHANGELOG.md` 的 `Unreleased` 内容收敛为 `v0.5.5 - 2026-07-18`。
...
- 本轮不修改 `main.go` 版本、不改 release 下载链接、不打 tag；正式版本准备另起一步完成。
...
- [x] `testloop-mcp --version` 输出 `testloop-mcp 0.5.4`，确认当前安装态仍是已发布版本。
- [x] 真实安装态 onboarding wrapper 输出 `agent_next_step=ready`。
