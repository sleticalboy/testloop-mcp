...
- [x] 版本准备提交远端 CI run `29644261865` passed。
- [x] Release Artifacts workflow `29644340675` passed，五平台资产和 `.sha256` 已上传。
- [x] `scripts/verify-release-assets.sh v0.5.5` 已确认 10 个 Release 资产完整。
- [x] GitHub Release 正文已更新为正式 v0.5.5 发布说明。
- [x] 仓库内 Formula 和 `sleticalboy/homebrew-tap` 已更新到 `0.5.5`，tap commit 为 `e945158`。
- [x] Post-Release Verify run `29644550322` passed。
- [x] 本机 Homebrew tap 快进后 `brew fetch` 已确认获取 `testloop-mcp (0.5.5)`。
...
- 正式发布已完成，Homebrew Formula 使用 Release API 返回的真实 asset digest；Windows 资产同样随 Release 发布，但 Homebrew Formula 只覆盖 macOS arm64、Linux amd64 和 Linux arm64。
...
- v0.5.5 已正式发布；发布证据包括 Release Artifacts run `29644340675`、资产校验、tap commit `e945158`、Post-Release Verify run `29644550322` 和本机 `brew fetch` 获取 `0.5.5`。
