...
这是 v0.4.14 的版本准备和正式发布记录。版本准备、tag、GitHub Release、Release Artifacts、资产校验、GitHub Release 正文、Homebrew tap 发布核验和 Post-Release Verify 均已完成。
...
- [x] 手动触发 Post-Release Verify，确认五平台安装脚本 dry run 全部通过：run `29157901152` passed。
...
- [x] `Post-Release Verify` workflow：run `29157901152` passed，资产清单和五平台安装脚本 dry run 全部通过。
- [x] 发布记录提交后的 main CI：run `29158887713` passed。

## 当前结论

v0.4.14 已正式发布，GitHub Release 资产、Homebrew 安装链路和 Post-Release Verify 五平台安装脚本 dry run 均已验证通过。
