...
6. 对 `failed/needs_better_input`，优先读取 `metadata.next_action_reason` 和 `metadata.coverage_missed_lines`，重新选择输入或公共入口；旧客户端可回退 `metadata.coverage_miss_reason`。
7. 客户端必须忽略未知字段。
...
如果要直接把 workflow 写入外部客户端仓库：

```bash
scripts/install-agent-decision-client-ci-template.sh /absolute/path/to/client-repo
```

这个安装脚本也支持脱离仓库单文件运行，适合用 `curl` 从 `main` raw URL 下载后写入客户端仓库；生成的 workflow 仍会固定到稳定 helper tag。
完整安装链路的 dry-run 入口是：
...
它会创建临时 Node 客户端项目，复制 release summary 和 renderer，并运行客户端自己的 `npm test --silent`。可复制目录结构见 [Agent 决策 release response 客户端接入](./agent-decision-release-response-client.md)。
...
for each item in manifest.fixtures:
  ...
  if action == needs_better_input:
    assert payload.metadata.next_action_kind == "coverage_target_miss"
    assert payload.metadata.next_action_reason exists
    assert payload.metadata.needs_better_input_reason exists
```
