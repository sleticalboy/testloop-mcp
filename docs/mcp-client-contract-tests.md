...
这条命令会创建一个最小客户端目录，导出 fixture 包，运行导出包的 `npm test --silent`，并输出 `agent_decision_client_status=passed`、`agent_decision_fixture_count=8` 和 validator JSON 路径。它适合放在客户端仓库的 smoke job 中，确认复制路径、包内脚本和 `status/action -> decision` 合同没有漂移。
