...
## v0.5.21 release response 接入方样板记录
...
这条记录补齐“接入方能照抄什么”的证据：安装脚本负责写入 contract 包和 workflow，样板 helper 负责把 JSON response 转成 Agent 容易消费的稳定键值或 JSON 输出，artifact verifier 负责在下载后确认证据包完整、自洽、可交给 Agent。
